...
function OthelloApp() {
  ...
  // ▼ 追加箇所: 初期レーティング（0〜1000で設定可能、初期値1000）
  ...
  // ▼ 変更箇所: 選択された初期レーティング(initRating)を保存
  ...
  const handleInvite = async (targetUid, mode) => {
    ...
    if (mode === 'room') {
      ...
      const newGameRef = await addDoc(getGamesCollection(), {
        ...
        player1: user.uid, player1Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 }, 
        player2: null, player2Data: null, resultProcessedBy: [], createdAt: Date.now()
      });
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    try {
      ...
      await updateDoc(getGameRef(gid), { 
        ...
        player2: aiId, 
        player2Data: { 
          name: aiData.name || (selectedAI.uid.includes('_strong_') ? `AI-Master-${parseInt(aiId.split('_').pop())+1}` : `AI-Player-${parseInt(aiId.split('_').pop())+1}`), 
          rating: aiData.rating || 1000, 
          highestRating: aiData.highestRating || 1000, 
          wins: aiData.wins||0, 
          losses: aiData.losses||0, 
          draws: aiData.draws||0,
          currentStreak: aiData.currentStreak || 0
        } 
      });
    } catch (e) { 
      console.error(e); 
  ...
  const startRankedMatch = async () => {
    ...
    try {
      ...
      if (joined) {
        ...
        const newGameRef = await addDoc(getGamesCollection(), {
          gameType: 'ranked', status: 'waiting', board: JSON.stringify(initB), turn: 'black',
          player1: user.uid, player1Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 }, player2: null, player2Data: null, resultProcessedBy: [], createdAt: Date.now()
        });
  ...
  const joinRoom = async (roomPin) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!roomDoc.exists()) {
          const initB = INITIAL_BOARD.map(r => [...r]);
          transaction.set(roomRef, { gameType: 'room', status: 'waiting', board: JSON.stringify(initB), turn: 'black', player1: user.uid, player1Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 }, player2: null, player2Data: null, createdAt: Date.now() });
          role = 'player';
        } else {
  ...
  // ▼ 変更箇所: 初回ログイン画面に初期レーティング選択機能(0〜1000)を追加
  if (isFirstLogin) {
    return (
      ...
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm mb-6 relative">
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'black' ? activeTheme.light : ''} w-[100px]`}>
            ...
            {(playMode === 'ranked' || playMode === 'room_spectator') && (
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'white' ? activeTheme.light : ''} w-[100px]`}>
            ...
            {(playMode === 'ranked' || playMode === 'room_spectator') && player2 && (
              <span className="text-[10px] text-gray-500 font-bold w-full text-center whitespace-nowrap">
