...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    if (isAnalysisOn && gameState && gameState.status === 'playing') {
      ...
      const timerId = setTimeout(() => {
        ...
        const turnIndex = playMode === 'history_analysis' ? analysisIndex : (localHistory.length > 0 ? localHistory.length - 1 : (gameState.boardHistory?.length ? gameState.boardHistory.length - 1 : 0));
        const evalForBlack = gameState.turn === 'black' ? evaluation : -evaluation;
        setEvalHistory(prev => {
          const newHistory = [...prev];
          newHistory[turnIndex] = clampScore(evalForBlack);
  ...
  }, [gameState?.board, gameState?.turn, playMode, isDeepAnalysis, gameState?.allowAiAnalysis, analysisIndex]);
