...
// ▼ シーズン切り替え（棋譜保存システム追加のためバージョン4へ更新）
...
// --- AI (Minimax) Logic ---
// ▼ 修正: 思考力をレーティングのみに比例させるように変更（totalGamesを削除）
  ...
  useEffect(() => {
    ...
    const processResult = async () => {
      try {
        ...
        // ▼ 修正: 履歴に boardHistory と turnHistory を保存
        const historyItem = { 
          id: Date.now(), 
      ...
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        {(playMode === 'research' || playMode === 'history_analysis') && (
          <div className="bg-white p-3 rounded-xl shadow-sm mb-4 border border-blue-100">
            ...
            {/* ▼ 追加箇所: 履歴解析モードでの実際の手の評価表示 */}
            {playMode === 'history_analysis' && researchData.actualMoveEval && (
              <div className={`mt-3 p-2 rounded-lg text-center text-xs font-bold ${
