...
// ▼ 変更箇所: currentStreak（連勝数）を引数に追加し、5連勝以上ならボーナスを加算する
const calcRatingChange = (myRate, oppRate, myStones, oppStones, isWin, isDraw, currentStreak = 0) => {
  const K = 48;
  ...
  if (isWin && currentStreak >= 5) {
    bonus += 10 + (currentStreak - 5) * 2; // 5連勝で+10、6連勝で+12...
  }
...
function OthelloApp() {
        ...
        try {
          ...
          if (!strongAiCheck.exists()) {
            for (let i = 0; i < 8; i++) {
              ...
              const lbSnap = await getDoc(getLeaderboardRef(uid));
              if (!lbSnap.exists()) {
                await setDoc(getLeaderboardRef(uid), { uid, name: aiName, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, isStrongAI: true, status: 'online' }).catch(()=>{});
                await setDoc(getProfileRef(uid), { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], theme: 'dark', isAI: true, isStrongAI: true }).catch(()=>{});
              }
            }
          }
          ...
          for (let pair = 0; pair < 2; pair++) {
            if (aiList.length >= 2) {
              ...
              setTimeout(async () => {
                ...
                const updates = [
                  {uid:a1.uid, myRate: a1.rating, oppRate: a2.rating, myStones, oppStones, isWin:isA1Win&&!isDraw, isDraw}, 
                  {uid:a2.uid, myRate: a2.rating, oppRate: a1.rating, myStones: oppStones, oppStones: myStones, isWin:!isA1Win&&!isDraw, isDraw}
                ];
                ...
                for (let target of updates) {
                   try {
                     ...
                     // ▼ 修正箇所: AIのレート計算時にも、AI自身の連勝数を渡してボーナスを適用する
                     const change = calcRatingChange(target.myRate, target.oppRate, target.myStones, target.oppStones, target.isWin, target.isDraw, currentStreak);
                     const newRating = Math.max(0, target.myRate + change);
                     ...
                     if (p) {
                       await updateDoc(getProfileRef(target.uid), {
                         ...
                         currentStreak: target.isWin ? currentStreak + 1 : 0, // AIの連勝も記録
  ...
  useEffect(() => {
    ...
    if (aiId?.startsWith('ai_bot_') && (gameState.player1 === user?.uid || gameState.player2 === user?.uid)) {
      ...
      const depth = getAiDepth(aiData?.rating || 1000, totalGames, isStrongAI);
  ...
  useEffect(() => {
    ...
    const processResult = async () => {
      try {
        ...
        // ▼ 修正箇所: 計算時に自分(プレイヤー)の現在の連勝数を渡してボーナスを適用させる
        const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, myStones, oppStones, isWin, isDraw, currentProfile.currentStreak || 0);
        ...
        if (oppUid?.startsWith('ai_bot_')) {
           try {
             ...
             // ▼ 修正箇所: AI側のレート更新時にもAI自身の連勝数を渡す
             const aiChange = calcRatingChange(currentAiRating, currentProfile.rating || 1000, oppStones, myStones, !isWin && !isDraw, isDraw, aiCurrentStreak);
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    let aiData = { name: isStrong ? `AI-Master-${parseInt(aiId.split('_').pop())+1}` : `AI-Player-${parseInt(aiId.split('_').pop())+1}`, rating: 1000, wins: 0, losses: 0, draws: 0 };
    try {
    ...
    try {
      await updateDoc(getGameRef(gid), { 
        status: 'playing', 
        player2: aiId, 
        player2Data: { name: aiData.name || (isStrong ? `AI-Master-${parseInt(aiId.split('_').pop())+1}` : `AI-Player-${parseInt(aiId.split('_').pop())+1}`), rating: aiData.rating || 1000, wins: aiData.wins||0, losses: aiData.losses||0, draws: aiData.draws||0 } 
      });
    } catch (e) { 
      console.error(e); 
      triggerCommError();
    ...
    if (gameId && playMode === 'ranked') {
      if (gameState?.status === 'waiting' || !gameState) {
        ...
        try { 
          ...
          const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, 0, 64, false, false, currentProfile.currentStreak || 0);
          const newRating = Math.max(0, (currentProfile.rating || 1000) + change);
