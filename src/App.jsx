...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        // AI 初期化（セキュリティルールに弾かれても止まらないように強牢化）
        try {
          for (let i = 0; i < 10; i++) {
            const uid = `ai_bot_${i}`;
            const aiName = `AI-Player-${i+1}`;
            const lbRef = getLeaderboardRef(uid);
            const lbSnap = await getDoc(lbRef);
            if (!lbSnap.exists()) {
              // リーダーボードには常に確実に載せる
              await setDoc(lbRef, { uid, name: aiName, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, status: 'online' });
              // プロフィールも作成を試みる（ルールで弾かれたら無視）
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    // AIデータ読み込み（ルールで弾かれた場合のフォールバック付き）
      ...
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm mb-6 relative">
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'black' ? activeTheme.light : ''}`}>
            {/* ▼エラー修正箇所：スタンプ送信者がいて、それが自分(player1)である場合のみ */}
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'white' ? activeTheme.light : ''}`}>
            {/* ▼エラー修正箇所：スタンプ送信者がいて、それが相手(player2)である場合のみ */}
