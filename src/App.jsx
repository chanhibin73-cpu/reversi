  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'leaderboard'), (snap) => {
      ...
      if (myIndex !== -1) {
        ...
        // 自分の日またぎリセット処理
        if (myLbData.dailyDate !== todayStr || !myLbData.dailyRank) {
      ...
      // ▼ 修正箇所：他の全ユーザー（AI含む）の「日付変更時のリセット」および「初期化直後の順位保存」を代行する
      data.forEach((u, idx) => {
        if (u.uid !== user?.uid && (u.dailyDate !== todayStr || !u.dailyRank)) {
          ...
          }, Math.random() * 8000 + 2000); // サーバーへの負荷集中を防ぐためランダム遅延
        }
      });
    });
          ...
          <div className="bg-white w-full h-[85%] rounded-t-3xl shadow-xl flex flex-col animate-in slide-in-from-bottom-full duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {leaderboardData.slice(0, 10).map((lbUser, idx) => {
                ...
                const lbRank = idx + 1;
                // ▼ 修正: 基準順位(dailyRank)が存在する場合のみ計算、なければ0
