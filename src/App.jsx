...
function OthelloApp() {
    ...
    const interval = setInterval(async () => {
      try {
        if (Math.random() < 0.60) { 
          ...
          snap.forEach(d => { 
            const data = d.data();
            // ▼ 修正箇所4: ステータスが 'online' のAIのみを抽出（重複対戦を防止）
  ...
  useEffect(() => {
    ...
    const processResult = async () => {
      try {
        ...
        // ▼ 修正箇所2: 最新のプロフィールをDBから再取得し、先祖返りを防ぐ
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    const aiIndex = Math.floor(Math.random() * 32); // 変更箇所3 (32人に)
