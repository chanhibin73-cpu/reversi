...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        // AI 初期化
        try {
          // ▼ 変更箇所: AIの生成確認を一番最後の48人目（ai_bot_47）で行う
          const aiCheck = await getDoc(getLeaderboardRef('ai_bot_47'));
          if (!aiCheck.exists()) {
            // ▼ 変更箇所: 48人に変更
            for (let i = 0; i < 48; i++) {
              const uid = `ai_bot_${i}`;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        // ▼ 変更箇所: AI同士の対戦発生確率を 0.80 (80%) に引き上げ
        if (Math.random() < 0.80) { 
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    // ▼ 変更箇所: AIの選択範囲を48人に変更
    const aiIndex = Math.floor(Math.random() * 48);
