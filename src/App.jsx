...
const minimax = (board, depth, alpha, beta, isMaximizing, color, origColor) => {
  ...
  if (moves.length === 0) {
    ...
    if (getSimulatedMoves(board, opp).length === 0) {
      ...
      // ▼ 修正箇所: 勝利確定時のスコアを ±1000 に統一
      return myCount > oppCount ? 1000 : (myCount < oppCount ? -1000 : 0);
    }
...
function OthelloApp() {
  ...
  const getEvaluationText = () => {
    ...
    let score = researchData.evaluation;
    ...
    if ((playMode === 'research' || playMode === 'history_analysis') && gameState && gameState.status === 'playing') {
      ...
      const timerId = setTimeout(() => {
        ...
        if (moves.length === 0) {
          ...
          let evaluation = myCount > oppCount ? 1000 : (myCount < oppCount ? -1000 : 0);
          ...
          setEvalHistory(prev => {
            ...
            newHistory[turnIndex] = evaluation;
            return newHistory;
          });
          return;
        }
  ...
  // ▼ 追加: AI解析モードの評価値折れ線グラフ描画
  const renderEvalGraph = () => {
    const w = 300;
    const h = 60;
    const maxTurn = 60;
    
    const validEvals = evalHistory.filter(v => v !== undefined);
    ...
    return (
      <div className="w-full h-[60px] bg-gray-50 border border-gray-200 rounded-lg relative overflow-hidden shrink-0 mt-1">
        ...
        <div className="absolute top-1 left-1 text-[8px] font-bold text-gray-400">+{absMax}</div>
        <div className="absolute bottom-1 left-1 text-[8px] font-bold text-gray-400">-{absMax}</div>
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
          <polyline points={validPoints} fill="none" stroke="#3b82f6" strokeWidth="2" strokeLinejoin="round" />
          {evalHistory[turnIndex] !== undefined && (
            <circle 
  ...
  const renderGame = () => {
    ...
    return (
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        {/* ▼ 修正: 解析エリアの高さを完全に固定し、動的スケールの折れ線グラフを表示 */}
