...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        // AI 初期化（プロフィールのエラーで止まらないようにLeaderboard基準に修正）
        try {
          const aiCheck = await getDoc(getLeaderboardRef('ai_bot_0'));
          if (!aiCheck.exists()) {
            for (let i = 0; i < 10; i++) {
              const uid = `ai_bot_${i}`;
          ...
          if (aiList.length >= 2) {
            ...
            setTimeout(async () => {
              ...
              for (let target of updates) {
                 // ▼ エラー修正：プロフィールが読み込めなくてもランキングデータは確実に更新する
    ...
    const processResult = async () => {
      try {
        ...
        if (oppUid?.startsWith('ai_bot_')) {
           try {
             // ▼ エラー修正：プレイヤー対AIの際にもAIのランキングデータを確実に更新
