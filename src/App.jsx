...
// --- AI (Minimax) Logic ---
// ▼ 変更箇所: 強化AI判定(isStrongAI)を受け取り、1.5倍(最大9手先)の深さを計算する
...
function OthelloApp() {
    ...
    const interval = setInterval(async () => {
      try {
        if (Math.random() < 0.60) { 
          ...
          snap.forEach(d => { 
            const data = d.data();
            // 強化AIも含めて抽出される（uidが'ai_bot_'から始まるため）
  ...
  useEffect(() => {
    ...
    const processResult = async () => {
      try {
        ...
        const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, myStones, oppStones, isWin, isDraw);
        ...
        if (oppUid?.startsWith('ai_bot_')) {
           try {
             ...
             const aiChange = calcRatingChange(currentAiRating, currentProfile.rating || 1000, oppStones, myStones, !isWin && !isDraw, isDraw);
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    // ▼ 追加箇所: 20%の確率で強化AI(AI-Master)をマッチングさせる
    ...
    try {
      await updateDoc(getGameRef(gid), { 
        status: 'playing', 
        player2: aiId, 
        player2Data: { name: aiData.name, rating: aiData.rating || 1000, wins: aiData.wins||0, losses: aiData.losses||0, draws: aiData.draws||0 } 
      });
    } catch (e) { 
      console.error(e); 
      triggerCommError();
    ...
    if (gameId && playMode === 'ranked') {
      if (gameState?.status === 'waiting' || !gameState) {
        ...
        try { 
          ...
          const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, 0, 64, false, false);
          const newRating = Math.max(0, (currentProfile.rating || 1000) + change);
