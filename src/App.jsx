...
function OthelloApp() {
      ...
      if (currentUser) {
        ...
        try {
          for (let i = 0; i < 10; i++) {
            ...
            if (!lbSnap.exists()) {
              await setDoc(lbRef, { uid, name: aiName, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, status: 'online' }).catch(() => {});
  ...
  // --- Leaderboard & AI vs AI 自動対戦シミュレーション ---
  useEffect(() => {
    if (!user || isFirstLogin) return; 
    ...
    const interval = setInterval(async () => {
      if (Math.random() < 0.3) { // 30%の確率でAI同士の対戦が発生
        try {
