...
function OthelloApp() {
  ...
  // ▼ 変更箇所: 形勢判断テキストを先手(黒)基準の絶対評価で表示するように修正
  const getEvaluationText = (evaluation, turn, isAnalyzing) => {
    if (isAnalyzing) return '計算中...';
    // evaluationは「現在の手番」から見たスコア。これを「黒」基準に変換。
    const blackScore = turn === 'black' ? evaluation : -evaluation;
    if (blackScore === 0) return '互角 (0)';
    return `${blackScore > 0 ? '+' : ''}${blackScore} (${blackScore > 0 ? '黒有利' : '白有利'})`;
  };
  ...
  }, [gameState?.board, gameState?.turn, playMode, isDeepAnalysis, analysisIndex]);
  ...
  const renderGame = () => {
    ...
    const blackScore = gameState?.turn === 'black' ? researchData.evaluation : -researchData.evaluation;
    let evalPercentage = 50;
    if (!researchData.isAnalyzing) {
       evalPercentage = 50 + (blackScore / 80) * 50;
       evalPercentage = Math.max(0, Math.min(100, evalPercentage));
       if (blackScore >= 9000) evalPercentage = 100;
       if (blackScore <= -9000) evalPercentage = 0;
    }
    ...
    return (
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        {(playMode === 'research' || playMode === 'history_analysis') && (
          <div className="bg-white p-3 rounded-xl shadow-sm mb-4 border border-blue-100 flex flex-col shrink-0" style={{ height: playMode === 'history_analysis' ? '185px' : '140px' }}>
            <div className="flex justify-between items-center mb-2 shrink-0 h-6">
              ...
              <span className={`text-sm font-black ${blackScore > 0 ? 'text-gray-800' : blackScore < 0 ? 'text-gray-400' : 'text-gray-500'}`}>
                形勢: {getEvaluationText(researchData.evaluation, turn, researchData.isAnalyzing)}
              </span>
            </div>

            <div className="w-full h-2.5 bg-gray-200 rounded-full overflow-hidden mb-3 relative flex shrink-0 shadow-inner">
              <div className="h-full bg-gray-800 transition-all duration-500 ease-out" style={{ width: `${evalPercentage}%` }}></div>
              <div className="h-full bg-white transition-all duration-500 ease-out border-y border-r border-gray-300" style={{ width: `${100 - evalPercentage}%` }}></div>
              <div className="absolute top-0 left-1/2 w-0.5 h-full bg-red-500 transform -translate-x-1/2 opacity-60"></div>
            </div>

            <div className="flex space-x-2 h-[48px] shrink-0">
              {researchData.topMoves.map((m, i) => {
                const displayScore = turn === 'black' ? m.score : -m.score;
                return (
                  <div key={i} className={`flex-1 text-center py-1.5 rounded-lg text-xs font-bold shadow-sm border ${i === 0 ? 'bg-red-50 text-red-600 border-red-200' : i === 1 ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                    {i + 1}位: {String.fromCharCode(65 + m.c)}{m.r + 1} <br/><span className="text-[10px] opacity-70">({displayScore > 0 ? '+' : ''}{displayScore})</span>
                  </div>
                );
              })}
            ...
            {playMode === 'history_analysis' && (
              <div className="h-[36px] mt-2 shrink-0">
                {researchData.actualMoveEval ? (
                  (() => {
                     const evalData = researchData.actualMoveEval;
                     const displayScore = turn === 'black' ? evalData.score : -evalData.score;
                     return (
                       <div className={`p-2 rounded-lg text-center text-xs font-bold h-full flex items-center justify-center ${
                         evalData.judgment === '最善手' ? 'bg-blue-100 text-blue-700' :
                         evalData.judgment === '好手' ? 'bg-emerald-100 text-emerald-700' :
                         evalData.judgment === '疑問手' ? 'bg-yellow-100 text-yellow-700' :
                         'bg-red-100 text-red-700'
                       }`}>
                         実際の指し手: {String.fromCharCode(65 + evalData.c)}{evalData.r + 1} 
                         <span className="opacity-70 ml-2">({displayScore > 0 ? '+' : ''}{displayScore} / {evalData.judgment})</span>
                       </div>
                     );
                  })()
