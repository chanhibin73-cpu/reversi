...
// ▼ 変更箇所: oppCurrentStreak を追加、Kファクターを72に引き上げ、連勝ストッパーボーナスを追加
...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        try {
          const strongAiCheck = await getDoc(getLeaderboardRef('ai_bot_strong_7'));
    ...
    const interval = setInterval(async () => {
      try {
        if (Math.random() < 0.60) { 
          ...
          snap.forEach(d => { 
            const data = d.data();
            if(data.uid?.startsWith('ai_bot_') && data.status === 'online') aiList.push(data); 
          });
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    const aiId = isStrong ? `ai_bot_strong_${Math.floor(Math.random() * 8)}` : `ai_bot_${Math.floor(Math.random() * 32)}`;
    
    let aiData = { name: isStrong ? `AI-Master-${parseInt(aiId.split('_').pop())+1}` : `AI-Player-${parseInt(aiId.split('_').pop())+1}`, rating: 1000, highestRating: 1000, wins: 0, losses: 0, draws: 0, currentStreak: 0 };
    try {
    ...
    try {
      await updateDoc(getGameRef(gid), { 
        ...
        player2: aiId, 
        player2Data: { 
          name: aiData.name || (isStrong ? `AI-Master-${parseInt(aiId.split('_').pop())+1}` : `AI-Player-${parseInt(aiId.split('_').pop())+1}`), 
