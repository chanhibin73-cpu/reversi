...
// 評価値を実際のコマ数差(±64)のスケールに変換する関数
const clampScore = (score) => {
  if (score >= 800) return Math.floor((score - 1000) / 10);
  if (score <= -800) return Math.ceil((score + 1000) / 10);
  return Math.max(-64, Math.min(64, Math.round(score / 10)));
};
...
function OthelloApp() {
  ...
  const [analysisMoveEvals, setAnalysisMoveEvals] = useState([]);
  ...
  const renderEvalGraph = () => {
    ...
    let absMax = validEvals.length > 0 ? Math.max(...validEvals.map(Math.abs)) : 10;
    if (absMax < 10) absMax = 10;
    if (absMax > 64) absMax = 64; 
      ...
      <div className="w-full h-[60px] bg-gray-50 border border-gray-200 rounded-lg relative overflow-hidden shrink-0 mt-1">
        ...
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
          ...
          {evalHistory[turnIndex] !== undefined && evalHistory[turnIndex] !== null && (
            <circle 
              cx={(turnIndex / maxTurn) * w} 
              cy={h/2 - (Math.max(-absMax, Math.min(absMax, evalHistory[turnIndex])) / absMax) * (h/2)} 
              r="3" fill="#ef4444" 
            />
          )}
        </svg>
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        } catch(e) { console.error("Strong AI Setup Exception:", e); }
  ...
  const updateProfile = async (updates) => {
    ...
    setProfile(newProfile);
    if (user) {
      await updateDoc(getProfileRef(user.uid), updates).catch(e => { console.error(e); triggerCommError(); });
  ...
  const handleInvite = async (targetUid, mode) => {
    ...
    if (mode === 'room') {
      ...
      await setDoc(getGameRef(newRoomPin), {
        ...
        player1: user.uid, player1Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 }, 
        player2: null, player2Data: null, createdAt: Date.now()
      });
      ...
      const newGameRef = await addDoc(getGamesCollection(), {
        ...
        boardHistory: [JSON.stringify(initB)], turnHistory: ['black'],
        player1: isCreatorP1 ? user.uid : null, 
        player1Data: isCreatorP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : null, 
        player2: !isCreatorP1 ? user.uid : null, 
        player2Data: !isCreatorP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : null, 
  ...
  // --- Game Sync ---
  useEffect(() => {
    if (!user || !gameId || playMode === 'local' || playMode === 'research' || playMode === 'history_analysis' || playMode === 'analyzing') return;
    const unsubscribe = onSnapshot(getGameRef(gameId), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        if (typeof data.board === 'string') data.board = JSON.parse(data.board);
        
        if (data.status === 'playing' && (!data.player1 || !data.player2 || !data.player1Data || !data.player2Data)) {
          triggerCommError();
          handleLeaveGame();
          return;
        }
  ...
  useEffect(() => {
    if (gameState?.status === 'playing' && matchTimeoutId) {
  ...
  // ▼ 一括解析実行関数
  const runFullAnalysis = async (item, mode) => {
    setShowAnalysisModal(null);
    setPlayMode('analyzing');
    setAnalysisProgress(0);

    const boards = item.boardHistory.map(b => JSON.parse(b));
    const turns = item.turnHistory || boards.map((_, i) => i % 2 === 0 ? 'black' : 'white');
    
    let resultEvals = [];
    let resultMoveEvals = [];

    const depthBase = mode === 'deep' ? 5 : 4;
    const endDepth = mode === 'deep' ? 10 : 8;
    ...
    for (let i = 0; i < boards.length; i++) {
      ...
      let emptyCount = 0;
      board.forEach(row => row.forEach(cell => { if (cell === null) emptyCount++; }));
      
      let currentDepth = depthBase;
      if (emptyCount <= endDepth) currentDepth = emptyCount;
      ...
      if (moves.length === 0) {
        ...
        const evalForBlack = turn === 'black' ? evaluation : -evaluation;
        resultEvals.push(clampScore(evalForBlack));
        resultMoveEvals.push({ topMoves: [], actualMoveEval: null });
      } else {
        let evaluatedMoves = [];
        for (let move of moves) {
          let newBoard = board.map(r => [...r]);
          ...
          let score = minimax(newBoard, currentDepth - 1, -Infinity, Infinity, false, turn === 'black' ? 'white' : 'black', turn);
        ...
        if (i < boards.length - 1) {
           const actualMove = getMoveFromBoards(board, boards[i + 1], turn);
           if (actualMove) {
        ...
        resultMoveEvals.push({ topMoves: evaluatedMoves.slice(0, 3), actualMoveEval });
      }

      setAnalysisProgress(Math.round(((i + 1) / boards.length) * 100));
    }

    const analysisData = { evals: resultEvals, moveEvals: resultMoveEvals, mode };
    const updatedHistory = profile.history.map(h => h.id === item.id ? { ...h, analysisData } : h);
    await updateProfile({ history: updatedHistory });

    setPlayMode(null);
    triggerCommError("AI一括解析が完了しました！結果をご覧ください。");
  };
  ...
  const viewHistoryAnalysis = (item) => {
    ...
    setAnalysisIndex(0);
    setEvalHistory(item.analysisData.evals);
    setAnalysisMoveEvals(item.analysisData.moveEvals);
    
    const turnColor = item.turnHistory ? item.turnHistory[0] : 'black';
    const turnData = item.analysisData.moveEvals[0] || { topMoves: [], actualMoveEval: null };
    const rawEval = item.analysisData.evals[0] || 0;
    const evalScore = turnColor === 'black' ? rawEval : -rawEval;
    
    setResearchData({
       topMoves: turnData.topMoves,
       evaluation: evalScore,
       isAnalyzing: false,
       actualMoveEval: turnData.actualMoveEval
    });
  ...
  const changeAnalysisIndex = (newIndex) => {
    ...
    const turnColor = analysisTurnHistory[newIndex] || 'black';
    setGameState(prev => ({
      ...prev,
    ...
    if (playMode === 'history_analysis') {
       const turnData = analysisMoveEvals[newIndex] || { topMoves: [], actualMoveEval: null };
       const rawEval = evalHistory[newIndex] || 0;
       const evalScore = turnColor === 'black' ? rawEval : -rawEval;
       setResearchData({
          topMoves: turnData.topMoves,
          evaluation: evalScore,
  ...
  // ▼ リアルタイム解析 (研究モード用)
  useEffect(() => {
    if (playMode === 'research' && gameState && gameState.status === 'playing') {
      ...
      const timerId = setTimeout(() => {
        ...
        let depth = isDeepAnalysis ? 5 : 4; 
        if (isDeepAnalysis && emptyCount <= 10) depth = emptyCount;
        else if (!isDeepAnalysis && emptyCount <= 8) depth = emptyCount;
        ...
        if (moves.length === 0) {
          ...
          const turnIndex = localHistory.length - 1;
        ...
        for (let move of moves) {
          ...
          let score = minimax(newBoard, depth - 1, -Infinity, Infinity, false, gameState.turn === 'black' ? 'white' : 'black', gameState.turn);
          score = clampScore(score);
          evaluatedMoves.push({ ...move, score });
        }
        evaluatedMoves.sort((a, b) => b.score - a.score);
  ...
  }, [gameState?.board, gameState?.turn, playMode, isDeepAnalysis]);
  ...
  useEffect(() => {
    ...
    const processResult = async () => {
      try {
        ...
        const oppCurrentStreak = oppData?.currentStreak || 0;
        const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, myStones, oppStones, isWin, isDraw, currentProfile.currentStreak || 0, false, oppCurrentStreak);
        ...
        await updateProfile({
          rating: newRating, 
          highestRating: Math.max(currentProfile.highestRating || 1000, newRating), 
          currentStreak: isWin && !isDraw ? (currentProfile.currentStreak || 0) + 1 : 0,
          wins: (currentProfile.wins || 0) + (isWin && !isDraw ? 1 : 0), 
          losses: (currentProfile.losses || 0) + (!isWin && !isDraw ? 1 : 0), 
          draws: (currentProfile.draws || 0) + (isDraw ? 1 : 0), 
        ...
        if (oppUid?.startsWith('ai_bot_')) {
           try {
             ...
             if(aiSnap.exists()) {
               aiP = aiSnap.data();
               aiCurrentStreak = aiP.currentStreak || 0;
             }

             const aiChange = calcRatingChange(currentAiRating, currentProfile.rating || 1000, oppStones, myStones, !isWin && !isDraw, isDraw, aiCurrentStreak, true, currentProfile.currentStreak || 0);
             const newAiRating = Math.max(0, currentAiRating + aiChange);
             const newAiHighestRating = Math.max(aiP?.highestRating || 1000, newAiRating);
             const newAiCurrentStreak = (!isWin && !isDraw) ? aiCurrentStreak + 1 : 0;
             const newAiWins = (aiP?.wins || 0) + (!isWin && !isDraw ? 1 : 0);
             const newAiLosses = (aiP?.losses || 0) + (isWin && !isDraw ? 1 : 0);
             const newAiDraws = (aiP?.draws || 0) + (isDraw ? 1 : 0);

             await updateDoc(getLeaderboardRef(oppUid), { 
               rating: newAiRating, status: 'online',
               highestRating: newAiHighestRating,
               currentStreak: newAiCurrentStreak,
               wins: newAiWins,
               losses: newAiLosses,
               draws: newAiDraws
             }).catch(()=>{});
             ...
             if(aiP) {
               await updateDoc(getProfileRef(oppUid), {
                 rating: newAiRating, highestRating: newAiHighestRating, currentStreak: newAiCurrentStreak, wins: newAiWins, losses: newAiLosses, draws: newAiDraws,
                 history: [historyItem, ...(aiP.history || [])].slice(0, 10)
               });
             }
  ...
  const renderGame = () => {
    if (playMode === 'analyzing') return (
      <div className="h-full flex flex-col items-center justify-center space-y-4 px-6 text-center animate-in fade-in">
        <Bot size={64} className="animate-bounce text-indigo-500" />
        <h2 className="text-xl font-black text-gray-800">全手AI解析中...</h2>
        <div className="w-full max-w-[250px] bg-gray-200 rounded-full h-3 mt-4 overflow-hidden shadow-inner">
          <div className="bg-indigo-500 h-full transition-all duration-300" style={{ width: `${analysisProgress}%` }}></div>
        </div>
        <p className="font-bold text-indigo-600">{analysisProgress}%</p>
        <p className="text-xs text-gray-400 mt-2">画面を閉じずにお待ちください<br/>(環境により数十秒〜数分かかります)</p>
      </div>
    );
    ...
    return (
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        {(playMode === 'research' || playMode === 'history_analysis') && (
          <div className="bg-white p-3 rounded-xl shadow-sm mb-4 border border-blue-100 flex flex-col shrink-0" style={{ height: playMode === 'history_analysis' ? '216px' : '172px' }}>
            <div className="flex justify-between items-center mb-2 shrink-0 h-6">
              <div className="flex items-center space-x-2">
                ...
                {playMode === 'research' && <button onClick={() => setIsDeepAnalysis(!isDeepAnalysis)} className={`text-[10px] px-2 py-0.5 rounded font-bold transition-colors ${isDeepAnalysis ? 'bg-indigo-500 text-white shadow-inner' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>超解析</button>}
              </div>
          ...
          <div className={`${activeTheme.board} p-1.5 rounded-lg shadow-2xl w-full max-w-[350px] aspect-square`}>
            <div className={`grid grid-cols-8 grid-rows-8 gap-0.5 h-full w-full ${activeTheme.grid} border-2 ${activeTheme.border}`}>
              {board.map((row, rIdx) => row.map((cell, cIdx) => {
                ...
                let rankIndex = -1;
                if ((playMode === 'research' || playMode === 'history_analysis') && !researchData.isAnalyzing) {
        ...
        {status === 'finished' && playMode !== 'research' && playMode !== 'history_analysis' && (
              ...
              {profile.history.map((item) => (
                <div key={item.id} className="bg-gray-50 p-3 rounded-xl">
                  ...
                  {item.boardHistory && item.boardHistory.length > 0 && (
                    !item.analysisData ? (
                      <button onClick={() => setShowAnalysisModal(item)} className="mt-2 px-3 py-2 w-full bg-indigo-50 text-indigo-600 rounded-lg text-xs font-bold shadow-sm border border-indigo-200 active:scale-95 flex items-center justify-center">
                        <Bot size={14} className="mr-1"/> この対局をAI一括解析する
                      </button>
                    ) : (
                      <button onClick={() => viewHistoryAnalysis(item)} className="mt-2 px-3 py-2 w-full bg-indigo-500 text-white rounded-lg text-xs font-bold shadow-sm active:scale-95 flex items-center justify-center">
                        <Bot size={14} className="mr-1"/> 🔍 解析結果を見る
                      </button>
                    )
                  )}
                </div>
  ...
  return (
    <div className="bg-gray-100 h-screen w-full flex justify-center font-sans overflow-hidden text-gray-800 select-none">
      <div className="w-full max-w-md h-full flex flex-col bg-gray-50 shadow-2xl relative">
        ...
        {showAnalysisModal && (
          <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 animate-in fade-in">
            <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-sm space-y-5">
              <div className="text-center">
                <div className="w-14 h-14 bg-indigo-100 rounded-full mx-auto flex items-center justify-center mb-2"><Bot size={28} className="text-indigo-600"/></div>
                <h3 className="font-black text-xl text-gray-800">AI一括解析</h3>
                <p className="text-xs text-gray-500 mt-2 font-bold leading-relaxed">この対局の全手をAIが自動解析し、形勢グラフと各ターンの最善手を記録します。</p>
              </div>
              <div className="space-y-3">
                <button onClick={() => runFullAnalysis(showAnalysisModal, 'normal')} className="w-full py-3.5 bg-white border border-indigo-200 text-indigo-700 font-bold rounded-xl active:scale-95 shadow-sm text-sm">
                  通常解析 (約30〜60秒)
                </button>
                <button onClick={() => runFullAnalysis(showAnalysisModal, 'deep')} className="w-full py-3.5 bg-indigo-500 text-white font-bold rounded-xl active:scale-95 shadow-sm text-sm">
                  超解析モード (約2〜5分)
                </button>
                <button onClick={() => setShowAnalysisModal(null)} className="w-full py-3 bg-gray-100 text-gray-500 font-bold rounded-xl active:scale-95 text-xs">キャンセル</button>
              </div>
            </div>
          </div>
        )}
