...
const getAiDepth = (rating, isStrongAI = false) => {
  let depth = 3 + Math.floor((rating - 1000) / 200);
  if (isStrongAI) {
    depth = Math.floor(depth * 1.5);
    return Math.max(1, Math.min(depth, 9));
  }
...
function OthelloApp() {
  ...
  const getEvaluationText = () => {
    ...
    const score = researchData.evaluation;
    if (score === 0) return '互角 (0)';
    const colorName = gameState?.turn === 'black' ? '黒' : '白';
    const isAdvantage = score > 0;
    return `${score > 0 ? '+' : ''}${score} (${isAdvantage ? colorName + '有利' : (colorName === '黒' ? '白' : '黒') + '有利'})`;
  };
  ...
  // ▼ 解析ロジック (研究モード ＆ 履歴解析モード) ＋ 評価値履歴の保存
  useEffect(() => {
    if ((playMode === 'research' || playMode === 'history_analysis') && gameState && gameState.status === 'playing') {
      setResearchData(prev => ({ ...prev, isAnalyzing: true, actualMoveEval: null }));
      const timerId = setTimeout(() => {
        const depth = isDeepAnalysis ? 7 : 5; 
        const moves = getSimulatedMoves(gameState.board, gameState.turn);
        if (moves.length === 0) {
          setResearchData({ topMoves: [], evaluation: 0, isAnalyzing: false, actualMoveEval: null });
          return;
        }
        ...
        const evaluation = evaluatedMoves.length > 0 ? evaluatedMoves[0].score : 0;
        ...
        if (playMode === 'history_analysis' && analysisIndex < analysisHistory.length - 1) {
           ...
           if (actualMove) {
              ...
              if (matched) {
                 ...
                 const flips = getFlippableStones(gameState.board, actualMove.r, actualMove.c, gameState.turn);
                 flips.forEach(([fr, fc]) => newBoard[fr][fc] = gameState.turn);
                 const score = minimax(newBoard, depth - 1, -Infinity, Infinity, false, gameState.turn === 'black' ? 'white' : 'black', gameState.turn);
        ...
        // ▼ 追加: 形勢グラフ用に絶対評価値（黒視点）を記録
        const absoluteEval = gameState.turn === 'black' ? evaluation : -evaluation;
        setEvalHistory(prev => {
          const newHist = [...prev];
          const currIdx = playMode === 'history_analysis' ? analysisIndex : localHistory.length - 1;
          newHist[currIdx] = absoluteEval;
          return newHist;
        });
  ...
  }, [gameState?.board, gameState?.turn, playMode, isDeepAnalysis, analysisIndex, localHistory.length]);
  ...
  const startHistoryAnalysis = (item) => {
    if (!item.boardHistory || item.boardHistory.length === 0) {
      triggerCommError("この対局の棋譜データがありません。");
      return;
    }
    ...
    setGameState({ 
      ...
      turn: item.turnHistory ? item.turnHistory[0] : 'black', 
  ...
  const changeAnalysisIndex = (newIndex) => {
    ...
    setGameState(prev => ({
      ...
      turn: analysisTurnHistory[newIndex] || 'black'
    }));
    setResearchData({ topMoves: [], evaluation: 0, isAnalyzing: true, actualMoveEval: null });
  };
  ...
  const handleMove = async (row, col, forceColor = null) => {
    ...
    if (playMode === 'local' || playMode === 'research') {
      ...
      setGameState({ ...gameState, board: newBoard, turn: nextTurn, status: nextStatus, passEvent, boardHistory: newBoardHistory, turnHistory: newTurnHistory }); 
      await updateDoc(getGameRef(gameId), { board: JSON.stringify(newBoard), boardHistory: newBoardHistory, turnHistory: newTurnHistory, turn: nextTurn, status: nextStatus, passEvent }).catch(e => { triggerCommError(); });
    }
  };
  ...
  const handleStamp = async (text) => {
    ...
    await updateDoc(getGameRef(gameId), { stamp: { sender: user.uid, text, time: Date.now() } }).catch(e => { triggerCommError(); });
  ...
  // ▼ 追加: 形勢推移の折れ線グラフ描画用コンポーネント
  const renderEvalGraph = () => {
    const currentIdx = playMode === 'history_analysis' ? analysisIndex : localHistory.length - 1;
    const historyData = evalHistory.slice(0, currentIdx + 1);
    ...
    const points = historyData.map((ev, i) => {
      if (ev === undefined || ev === null) return null;
      const x = paddingX + (i / Math.max(1, historyData.length - 1)) * (w - paddingX * 2);
      const clippedEv = Math.max(minEval, Math.min(maxEval, ev));
      const yVal = (clippedEv - minEval) / range;
      const y = paddingY + (1 - yVal) * (h - paddingY * 2);
      return `${x},${y}`;
    }).filter(p => p !== null).join(' ');

    const y0 = paddingY + (1 - (0 - minEval) / range) * (h - paddingY * 2);

    return (
      <div className="w-full h-16 mt-3 bg-white rounded-xl relative border border-gray-200 shadow-inner">
         <span className="absolute top-1 left-2 text-[8px] font-bold text-gray-400">黒有利</span>
         <span className="absolute bottom-1 left-2 text-[8px] font-bold text-gray-400">白有利</span>
         <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-full overflow-visible">
            <line x1={paddingX} y1={y0} x2={w - paddingX} y2={y0} stroke="#cbd5e1" strokeWidth="1" strokeDasharray="2 2" />
            ...
            {historyData.map((ev, i) => {
               ...
               const yVal = (clippedEv - minEval) / range;
               const y = paddingY + (1 - yVal) * (h - paddingY * 2);
               return <circle key={i} cx={x} cy={y} r="2" fill={i === currentIdx ? "#ef4444" : "#6366f1"} />
            })}
         </svg>
      </div>
    );
  ...
  if (isFirstLogin) {
    return (
      <div className="bg-gray-100 h-screen w-full flex justify-center font-sans overflow-hidden text-gray-800">
        <div className="w-full max-w-md h-full flex flex-col bg-gray-50 shadow-2xl relative items-center justify-center p-6 space-y-6 animate-in fade-in duration-500">
          <div className="text-center space-y-2">
             <div className="w-20 h-20 bg-emerald-600 rounded-3xl mx-auto shadow-lg flex items-center justify-center transform rotate-3"><Swords size={40} className="text-white" /></div>
             <h1 className="text-2xl font-black text-gray-800 tracking-tight">REVERSI</h1>
             <p className="text-xs text-gray-500 font-bold">オンライン対戦へようこそ！</p>
          </div>
          <div className="w-full bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-5">
             <div className="space-y-2">
               <label className="text-xs font-bold text-gray-700 ml-1">プレイヤー名を入力</label>
               <input type="text" maxLength={10} value={initName} onChange={(e) => setInitName(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-base font-bold text-center" placeholder="名前 (10文字以内)" autoFocus />
             </div>

             <div className="space-y-2 pt-2 border-t border-gray-100">
               <div className="flex justify-between items-center">
                 <label className="text-xs font-bold text-gray-700 ml-1">初期レーティングを選択</label>
                 <span className="text-sm font-black text-emerald-600 font-mono">Rate: {initRating}</span>
               </div>
               <input 
                 type="range" 
                 min="0" 
                 ...
                 step="50" 
                 value={initRating} 
                 onChange={(e) => setInitRating(Number(e.target.value))} 
             ...
             <button onClick={handleInitialRegistration} disabled={initName.trim() === ''} className="w-full py-3.5 bg-emerald-600 text-white rounded-xl font-black text-base shadow-sm disabled:opacity-50 disabled:bg-gray-400 active:scale-95 transition-all flex items-center justify-center">はじめる <ChevronRightCircle size={18} className="ml-2" /></button>
          </div>
          <p className="text-[10px] text-gray-400 text-center">名前は後から設定画面で変更できます。</p>
        </div>
      </div>
    );
  }
  ...
  const renderGame = () => {
    ...
    return (
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        {(playMode === 'research' || playMode === 'history_analysis') && (
          <div className="bg-white p-3 rounded-xl shadow-sm mb-4 border border-blue-100">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`text-sm font-black ${researchData.evaluation > 0 ? (turn === 'black' ? 'text-gray-800' : 'text-gray-400') : researchData.evaluation < 0 ? (turn === 'black' ? 'text-gray-400' : 'text-gray-800') : 'text-gray-500'}`}>
            ...
            <div className="flex space-x-2">
              {researchData.topMoves.map((m, i) => (
                <div key={i} className={`flex-1 text-center py-1.5 rounded-lg text-xs font-bold shadow-sm border ${i === 0 ? 'bg-red-50 text-red-600 border-red-200' : i === 1 ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                  {i + 1}位: {String.fromCharCode(65 + m.c)}{m.r + 1} <br/><span className="text-[10px] opacity-70">({m.score > 0 ? '+' : ''}{m.score})</span>
                </div>
              ))}
            ...
            {/* ▼ 修正箇所: 履歴解析モードでの実際の指し手表示 (高さ固定でレイアウトシフト防止) */}
            {playMode === 'history_analysis' && (
              <div className="mt-2 h-7 flex items-center justify-center">
                {researchData.actualMoveEval ? (
                  <div className={`w-full py-1.5 rounded-lg text-center text-xs font-bold ${
                    ...
                    <span className="opacity-70 ml-2">({researchData.actualMoveEval.score > 0 ? '+' : ''}{researchData.actualMoveEval.score} / {researchData.actualMoveEval.judgment})</span>
                  </div>
                ) : (
                  <div className="w-full py-1.5 opacity-0 text-xs">プレースホルダー</div>
                )}
              </div>
            )}
        ...
        {playMode === 'history_analysis' && (
          <div className="mt-6 flex justify-between items-center bg-gray-100 p-2 rounded-xl space-x-2">
            <button onClick={() => changeAnalysisIndex(analysisIndex - 1)} disabled={analysisIndex <= 0 || researchData.isAnalyzing} className="flex-1 py-3 bg-white rounded-lg shadow-sm font-bold text-gray-600 active:scale-95 disabled:opacity-50">＜ 前の手</button>
            <div className="font-black text-gray-600 text-sm px-4">{analysisIndex}手目</div>
            <button onClick={() => changeAnalysisIndex(analysisIndex + 1)} disabled={analysisIndex >= analysisHistory.length - 1 || researchData.isAnalyzing} className="flex-1 py-3 bg-white rounded-lg shadow-sm font-bold text-gray-600 active:scale-95 disabled:opacity-50">次の手 ＞</button>
          </div>
        )}
        ...
        {playMode !== 'history_analysis' && (
          <div className="mt-6 flex space-x-3 relative">
            ...
            {/* ▼ 修正箇所: 待ったボタンを押した際に evalHistory も巻き戻す */}
            {(playMode === 'local' || playMode === 'research') && (<button onClick={() => { const h = localHistory.slice(0, -1); setLocalHistory(h); setGameState({ ...gameState, board: h[h.length - 1], turn: gameState.turn === 'black' ? 'white' : 'black', status: 'playing', passEvent: null }); setEvalHistory(prev => prev.slice(0, -1)); }} disabled={localHistory.length <= 1 || (status !== 'playing' && playMode !== 'research')} className={`flex-1 text-white py-3 rounded-xl font-bold active:scale-95 disabled:opacity-50 ${activeTheme.primary}`}>待った</button>)}
          </div>
        )}
  ...
  const renderGraph = (history, key, isReverse = false) => {
    const data = [...(history || [])];
    ...
    return (
      <div className="p-4 space-y-5 min-h-full animate-in fade-in duration-300 pb-8">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-2">
          ...
          {renderGraph(profile.ratingHistory, 'rating', false)}
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-2">
          ...
          {renderGraph(profile.ratingHistory, 'rank', true)}
        </div>
              ...
              {profile.history.map((item) => (
                <div key={item.id} className="bg-gray-50 p-3 rounded-xl">
                  ...
                  {item.boardHistory && item.boardHistory.length > 0 && (
                    <button onClick={() => startHistoryAnalysis(item)} className="mt-2 px-3 py-2 w-full bg-indigo-50 text-indigo-600 rounded-lg text-xs font-bold shadow-sm border border-indigo-200 active:scale-95 flex items-center justify-center">
                      <Bot size={14} className="mr-1"/> この対局をAI解析する
                    </button>
                  )}
                </div>
              ))}
