...
const getAiDepth = (rating, totalGames, isStrongAI = false) => {
  let baseDepth = 3 + Math.floor((rating - 1000) / 200) + Math.floor(totalGames / 100);
  if (isStrongAI) {
    // 強化AIは思考力1.5倍、最大9手
    return Math.max(1, Math.min(Math.floor(baseDepth * 1.5), 9));
  }
  // 通常AIは最大5手
  return Math.max(1, Math.min(baseDepth, 5));
};
...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        try {
          // 強化AIの有無で初期化判定
          const aiCheck = await getDoc(getProfileRef('ai_strong_bot_0'));
          if (!aiCheck.exists()) {
            // 1. 通常AI
            for (let i = 0; i < 10; i++) {
              const uid = `ai_bot_${i}`;
              const aiName = `AI-Player-${i+1}`;
              const p = { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], theme: 'dark', isAI: true, isStrongAI: false };
              await setDoc(getProfileRef(uid), p).catch(() => {});
              await setDoc(getLeaderboardRef(uid), { uid, name: aiName, rating: p.rating, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, status: 'online' }).catch(() => {});
            }
            // 2. 強化AI
            for (let i = 0; i < 2; i++) {
              const uid = `ai_strong_bot_${i}`;
              const aiName = `AI-Master-${i+1}`;
              const p = { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], theme: 'dark', isAI: true, isStrongAI: true };
              await setDoc(getProfileRef(uid), p).catch(() => {});
              await setDoc(getLeaderboardRef(uid), { uid, name: aiName, rating: p.rating, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, isStrongAI: true, status: 'online' }).catch(() => {});
            }
          }
  ...
  const handleInitialRegistration = async () => {
    ...
    try {
      ...
      setInitError("登録に失敗しました。通信環境やセキュリティルールの設定をご確認ください。");
    }
  };
    ...
    const interval = setInterval(async () => {
      if (Math.random() < 0.15) { 
        try {
          ...
          let aiList = [];
          // 通常AI・強化AI問わず対象とする
          snap.forEach(d => { if(d.data().uid?.includes('ai_')) aiList.push(d.data()); });
          if (aiList.length >= 2) {
            ...
            setTimeout(async () => {
              ...
              for (let target of [{uid:a1.uid, change:a1Change, isWin:isA1Win&&!isDraw, isDraw}, {uid:a2.uid, change:a2Change, isWin:!isA1Win&&!isDraw, isDraw}]) {
                 try {
                   ...
                   const p = pSnap.exists() ? pSnap.data() : { rating: 1000, wins: 0, losses: 0, draws: 0 };
                   const newRating = Math.max(0, (p.rating || 1000) + target.change);
                   await updateDoc(getProfileRef(target.uid), {
                     rating: newRating, highestRating: Math.max(p.highestRating || 1000, newRating),
                     wins: (p.wins || 0) + (target.isWin ? 1 : 0), losses: (p.losses || 0) + (!target.isWin && !target.isDraw ? 1 : 0), draws: (p.draws || 0) + (target.isDraw ? 1 : 0)
                   }).catch(()=>{});
  ...
  useEffect(() => {
    ...
    if (aiId?.includes('ai_') && (gameState.player1 === user?.uid || gameState.player2 === user?.uid)) {
      ...
      const isStrongAI = aiId.includes('strong_bot');
  ...
  useEffect(() => {
    ...
    const processResult = async () => {
      ...
      if (oppUid?.includes('ai_')) {
         try {
           const aiSnap = await getDoc(getProfileRef(oppUid));
           const aiP = aiSnap.exists() ? aiSnap.data() : { rating: 1000, wins: 0, losses: 0, draws: 0 };
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    // 全12人のAIから選出
    const aiIds = Array.from({length: 10}, (_, i) => `ai_bot_${i}`).concat(['ai_strong_bot_0', 'ai_strong_bot_1']);
    const aiId = aiIds[Math.floor(Math.random() * aiIds.length)];
    
    let aiData = { name: aiId.includes('strong') ? 'AI-Master' : 'AI-Player', rating: 1000, wins: 0, losses: 0, draws: 0 };
    try {
      const aiProfileSnap = await getDoc(getProfileRef(aiId));
      if (aiProfileSnap.exists()) aiData = aiProfileSnap.data();
    } catch(e) { console.warn("AI Read Rules Error", e); }

    await updateDoc(getGameRef(gid), { 
      status: 'playing', 
      player2: aiId, 
      player2Data: { name: aiData.name, rating: aiData.rating, wins: aiData.wins||0, losses: aiData.losses||0, draws: aiData.draws||0, isStrongAI: aiId.includes('strong') } 
    });
  };
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm mb-6 relative">
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'black' ? activeTheme.light : ''}`}>
            ...
            <div className="w-8 h-8 bg-black rounded-full shadow-md mb-1 border-2 border-transparent flex items-center justify-center">
              {player1?.includes('ai_') && <span className="text-[10px] text-white font-mono opacity-50">{player1.includes('strong') ? 'AI+' : 'AI'}</span>}
            </div>
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'white' ? activeTheme.light : ''}`}>
            ...
            <div className="w-8 h-8 bg-white rounded-full shadow-md mb-1 border-2 border-gray-200 flex items-center justify-center">
              {player2?.includes('ai_') && <span className="text-[10px] text-black font-mono opacity-50">{player2.includes('strong') ? 'AI+' : 'AI'}</span>}
            </div>
    ...
    if (this.state.hasError) {
      return (
        <div className="p-6 bg-red-50 text-red-900 min-h-screen w-full overflow-auto text-left">
          ...
          <p className="mt-6 text-xs text-red-500 font-bold">こちらの画面のスクリーンショットか、エラーの文字を教えてください。</p>
        </div>
      );
    }
