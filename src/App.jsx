...
// ▼ 修正: タイムアウト(endTime)引数を追加し、超過した場合は例外で脱出する
        ...
        try {
          ...
          for (let d = startDepth; d <= targetDepth; d++) {
            ...
            for (let move of moves) {
              ...
              if (score <= -500) score = -500;
              tempEvaluatedMoves.push({ ...move, score });
            }
            tempEvaluatedMoves.sort((a, b) => b.score - a.score);
            bestEvaluatedMoves = tempEvaluatedMoves;
        ...
        if (playMode === 'history_analysis' && analysisIndex < analysisHistory.length - 1) {
           ...
           if (actualMove) {
              ...
              if (matched) {
                 ...
                 let score = minimax(newBoard, 2, -Infinity, Infinity, false, gameState.turn === 'black' ? 'white' : 'black', gameState.turn);
                 if (score >= 500) score = 500;
                 if (score <= -500) score = -500;
    ...
    try {
      ...
      snap.forEach(doc => {
        ...
        if (data.status === 'waiting' && data.gameType === 'ranked') {
          // ▼ 修正: AI乱入時間を20秒(20000ms)に変更
          if (data.createdAt && (now - data.createdAt > 20000)) {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!roomDoc.exists()) {
          ...
          transaction.set(roomRef, { 
            ...
            player2Data: !isP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : null, 
            createdAt: Date.now(),
            allowAiAnalysis: allowAiAnalysis // ▼ ホスト設定を保存
          });
          role = 'player';
        } else {
