...
// ▼ シーズン切り替え（全データリセット＆AI構成変更のためバージョン3へ更新）
...
const calcRatingChange = (myRate, oppRate, myStones, oppStones, isWin, isDraw, currentStreak = 0, isAI = false, oppCurrentStreak = 0) => {
  // Eloレーティング計算式: レート差が離れるほど、上位者は上がりづらく下がりやすい。下位者は下がりづらく上がりやすい。
  const K = 72;
  ...
  if (!isWin && !isDraw && currentStreak >= 5) {
...
function OthelloApp() {
        ...
        if (Math.random() < 0.60) { 
          ...
          for (let pair = 0; pair < 2; pair++) {
            if (aiList.length >= 2) {
              ...
              let a2 = null;
              // ▼ 修正箇所: AI同士でも極力レートが近い(差が800未満)相手を探す
              ...
              // ▼ レート差が800以上離れていたら80%の確率でマッチングを見送る（しづらくする）
  ...
  const startRankedMatch = async () => {
    ...
    try {
      ...
      snap.forEach(doc => {
        ...
        if (data.status === 'waiting' && data.gameType === 'ranked') {
          if (data.createdAt && (now - data.createdAt > 60000)) {
            ...
            // ▼ 修正: レート差800未満を優先。800以上はバックアップとし、基本はマッチングを避ける
      ...
      // 800未満が居らず、800以上の部屋がある場合、10%の確率でのみ入室する（マッチングしづらくする）
