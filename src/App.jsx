...
// ▼ シーズン切り替え（全データリセット）用のバージョン管理（2に更新！）
...
const calcRatingChange = (myRate, oppRate, myStones, oppStones, isWin, isDraw, currentStreak = 0, isAI = false) => {
  const K = 48;
...
function OthelloApp() {
      ...
      if (myIndex !== -1) {
        ...
        if (myLbData.dailyDate !== todayStr) {
          ...
          setProfile(prev => {
            ...
            const newHistory = [...(prev.ratingHistory || []), newRecord].slice(-5); // 過去5日分
          ...
          for (let pair = 0; pair < 2; pair++) {
            if (aiList.length >= 2) {
              ...
              setTimeout(async () => {
                ...
                const a1Change = calcRatingChange(p1Data.rating, p2Data.rating, myStones, oppStones, isA1Win && !isDraw, isDraw, p1Data.currentStreak, true);
                const a2Change = calcRatingChange(p2Data.rating, p1Data.rating, oppStones, myStones, !isA1Win && !isDraw, isDraw, p2Data.currentStreak, true);
  ...
  useEffect(() => {
    ...
    if (aiId?.startsWith('ai_bot_') && (gameState.player1 === user?.uid || gameState.player2 === user?.uid)) {
      ...
      // ▼ 修正箇所: AIの思考力は現在のレーティング＋試合数を基準に戻す
  ...
  useEffect(() => {
    ...
    const processResult = async () => {
      try {
        ...
        const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, myStones, oppStones, isWin, isDraw, currentProfile.currentStreak || 0, false);
        ...
        if (oppUid?.startsWith('ai_bot_')) {
           try {
             ...
             const aiChange = calcRatingChange(currentAiRating, currentProfile.rating || 1000, oppStones, myStones, !isWin && !isDraw, isDraw, aiCurrentStreak, true);
    ...
    if (gameId && playMode === 'ranked') {
      if (gameState?.status === 'waiting' || !gameState) {
        ...
        try { 
          ...
          const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, 0, 64, false, false, currentProfile.currentStreak || 0, false);
          const newRating = Math.max(0, (currentProfile.rating || 1000) + change);
          ...
          {displayData.map((d, i) => {
            ...
            return (
              <g key={i}>
                 ...
                 <text x={x} y={h + 16} fontSize="8" fill="#9ca3af" textAnchor="middle">{formatDate(d.date)}</text>
              </g>
            )
          })}
        </svg>
    ...
    return (
      <div className="p-4 space-y-5 min-h-full animate-in fade-in duration-300 pb-8">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-2">
          <h3 className="font-bold text-gray-800 text-sm flex items-center"><BarChart2 size={16} className="mr-1.5 text-emerald-500" /> レーティング推移 (過去5日)</h3>
          {renderGraph(profile.ratingHistory, 'rating', false)}
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-2">
          <h3 className="font-bold text-gray-800 text-sm flex items-center"><BarChart2 size={16} className="mr-1.5 text-emerald-500" /> 順位推移 (過去5日)</h3>
