...
// ▼ 修正箇所: 空きマスの数(進行度)に応じて評価基準を変える高度な評価関数
const evaluateBoard = (board, color) => {
  ...
  // 完全終盤（空きマス12以下）は純粋に石の多さだけで評価する（スケール調整のため10倍）
  if (emptyStones <= 12) {
  ...
  // 中盤以降は場所の価値だけでなく、石の差の比重を少しずつ上げる
  if (emptyStones <= 30) {
...
const minimax = (board, depth, alpha, beta, isMaximizing, color, origColor) => {
  ...
  if (moves.length === 0) {
    ...
    if (getSimulatedMoves(board, opp).length === 0) {
      ...
      // ▼ 修正箇所: 勝利時は「1000＋石差」を返し、より多く取る手を内部で優先させる（表示時は500に制限する）
...
// ▼ 追加: 表示用のスコア制限関数（内部計算は1000以上でも、画面には±500で表示する）
    ...
    if ((playMode === 'research' || playMode === 'history_analysis') && gameState && gameState.status === 'playing') {
      ...
      const timerId = setTimeout(() => {
        ...
        if (moves.length === 0) {
          ...
          let evaluation = myCount > oppCount ? 1000 + (myCount - oppCount)*10 : (myCount < oppCount ? -1000 - (oppCount - myCount)*10 : 0);
