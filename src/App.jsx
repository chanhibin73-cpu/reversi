...
function OthelloApp() {
        ...
        try {
          ...
          if (!aiCheck.exists() || needsReset) {
            for (let i = 0; i < 20; i++) {
              ...
              await setDoc(getLeaderboardRef(uid), { uid, name: aiName, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, status: 'online', resetVersion: RESET_VERSION }).catch(()=>{});
        ...
        try {
          ...
          if (!strongAiCheck.exists() || needsReset) {
            for (let i = 0; i < 4; i++) {
              ...
              await setDoc(getLeaderboardRef(uid), { uid, name: aiName, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, isStrongAI: true, status: 'online', resetVersion: RESET_VERSION }).catch(()=>{});
        ...
        try {
          ...
          if (pSnap.exists()) {
            ...
            if (pData.resetVersion !== RESET_VERSION) {
              ...
              await setDoc(getLeaderboardRef(currentUser.uid), { uid: currentUser.uid, name: pData.name, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, status: 'online', resetVersion: RESET_VERSION }, { merge: true });
            }
  ...
  const handleInitialRegistration = async () => {
    ...
    try {
      ...
      await setDoc(getLeaderboardRef(user.uid), { uid: user.uid, name: newProfile.name, rating: newProfile.rating, dailyDate: todayStr, dailyRating: newProfile.rating, dailyRank: 0, status: 'online', resetVersion: RESET_VERSION });
  ...
  const handleUserClick = async (targetUser) => {
    ...
    try {
      ...
      const snap = await getDoc(getProfileRef(targetUser.uid));
      if (snap.exists()) {
                ...
                for (let target of updates) {
                   ...
                   try {
                     await updateDoc(getProfileRef(target.uid), {
                       rating: newRating, highestRating: Math.max(target.pData.highestRating || 1000, newRating),
                       currentStreak: target.isWin ? (target.pData.currentStreak || 0) + 1 : 0,
                       wins: (target.pData.wins || 0) + (target.isWin ? 1 : 0), 
                       losses: (target.pData.losses || 0) + (!target.isWin && !target.isDraw ? 1 : 0), 
                       draws: (target.pData.draws || 0) + (target.isDraw ? 1 : 0)
                     });
                   } catch(e) {} 
                }
              }, 4000); 
           ...
           try {
             ...
             if(aiP) {
               await updateDoc(getProfileRef(oppUid), {
                 ...
                 currentStreak: (!isWin && !isDraw) ? aiCurrentStreak + 1 : 0,
  ...
  const startResearchMode = () => {
    ...
    setIsDeepAnalysis(false); // 初期は通常解析
  };
    ...
    try {
      ...
      snap.forEach(doc => {
        ...
        if (data.status === 'waiting' && data.gameType === 'ranked') {
          ...
          } else if (data.player1 && data.player1Data && data.player1 !== user.uid) {
            if (Math.abs(data.player1Data.rating - profile.rating) < 800) {
