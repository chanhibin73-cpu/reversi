...
const calcRatingChange = (myRate, oppRate, myStones, oppStones, isWin, isDraw, currentStreak = 0, isAI = false) => {
  ...
  if (isWin && currentStreak >= 5) {
    let winBonus = 10 + (currentStreak - 5) * 2; // 5連勝で+10、6連勝で+12...
    if (winBonus > 50) winBonus = 50; // 最大ボーナスは50に制限
    if (isAI) winBonus = winBonus / 2; // AIプレイヤーの連勝ボーナスは1/2
    bonus += winBonus;
  }
  ...
  // 5連勝以上している際に負けた(連勝が崩れた)時は通常の1.2倍レートが下がる
...
function OthelloApp() {
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (!aiCheck.exists() || needsReset) {
            for (let i = 0; i < 32; i++) {
              ...
              await setDoc(getProfileRef(uid), { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], theme: 'dark', isAI: true, resetVersion: RESET_VERSION }).catch(()=>{});
            }
          }
        ...
        try {
          ...
          if (!strongAiCheck.exists() || needsReset) {
            for (let i = 0; i < 8; i++) {
              ...
              await setDoc(getProfileRef(uid), { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], theme: 'dark', isAI: true, isStrongAI: true, resetVersion: RESET_VERSION }).catch(()=>{});
            }
          }
        ...
        try {
          ...
          if (pSnap.exists()) {
            ...
            // バージョンが違う場合は強制リセット（1000へ戻す）
            if (pData.resetVersion !== RESET_VERSION) {
              pData = { ...pData, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], resetVersion: RESET_VERSION };
            ...
            setProfile({ ...pData, friends: pData.friends || [] });
  ...
  const handleInitialRegistration = async () => {
    ...
    setInitError(null);
    const newProfile = { name: initName.trim(), rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], friends: [], theme: 'green', resetVersion: RESET_VERSION };
          ...
          for (let pair = 0; pair < 2; pair++) {
            if (aiList.length >= 2) {
              ...
              setTimeout(async () => {
                ...
                // ▼ 修正: 計算時にAI側の連勝数と isAI=true を渡してボーナスを適用させる
  ...
  useEffect(() => {
    ...
    if (aiId?.startsWith('ai_bot_') && (gameState.player1 === user?.uid || gameState.player2 === user?.uid)) {
      ...
      // ▼ 修正: AIの思考力はAIの「最高レーティング(highestRating)」を基準に計算する
      const referenceRating = aiData?.highestRating || aiData?.rating || 1000;
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    let aiData = { name: isStrong ? `AI-Master-${parseInt(aiId.split('_').pop())+1}` : `AI-Player-${parseInt(aiId.split('_').pop())+1}`, rating: 1000, highestRating: 1000, wins: 0, losses: 0, draws: 0 };
    try {
    ...
    try {
          ...
          await runTransaction(db, async (transaction) => {
            ...
            if (gameDoc.exists() && gameDoc.data().status === 'waiting') {
              transaction.update(getGameRef(foundGame.id), { 
                status: 'playing', 
                player2: user.uid, 
                player2Data: { name: profile.name, rating: profile.rating } 
              });
              joined = true;
            }
          });
      ...
      if (joined) {
        ...
        const newGameRef = await addDoc(getGamesCollection(), {
          ...
          player1: user.uid, player1Data: { name: profile.name, rating: profile.rating }, player2: null, player2Data: null, resultProcessedBy: [], createdAt: Date.now()
        });
  ...
  const joinRoom = async (roomPin) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!roomDoc.exists()) {
          const initB = INITIAL_BOARD.map(r => [...r]);
          transaction.set(roomRef, { gameType: 'room', status: 'waiting', board: JSON.stringify(initB), turn: 'black', player1: user.uid, player1Data: { name: profile.name }, player2: null, player2Data: null, createdAt: Date.now() });
          role = 'player';
        } else {
