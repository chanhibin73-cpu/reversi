...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const now = new Date();
        const isRestTime = now.getHours() === 23 && now.getMinutes() >= 45;
        ...
        if (isRestTime) {
          // 休憩時間処理: 試合中以外のAIをオフライン(赤)にする
          aiList.forEach(ai => {
            if (ai.status !== 'playing' && ai.status !== 'offline') {
              updateDoc(getLeaderboardRef(ai.uid), { status: 'offline' }).catch(()=>{});
            }
          });
          return; // 休憩中は新規シミュレーションを発生させない
        } else {
          // 休憩終了後: オフラインになっているAIをオンライン(緑)に戻す
          aiList.forEach(ai => {
            if (ai.status === 'offline') {
              updateDoc(getLeaderboardRef(ai.uid), { status: 'online' }).catch(()=>{});
            }
          });
        }
          ...
          if (aiList.length >= 2) {
            ...
            setTimeout(async () => {
              ...
              for (let target of updates) {
                 ...
                 // 試合終了時に休憩時間ならオフラインにする
                 const n = new Date();
                 const isRest = n.getHours() === 23 && n.getMinutes() >= 45;

                 await updateDoc(getLeaderboardRef(target.uid), { rating: newRating, status: isRest ? 'offline' : 'online' }).catch(()=>{});
        ...
        if (moveStr) {
          ...
          handleMove(r, c, gameState.turn);
           ...
           if (getValidMoves(gameState.board, gameState.turn).length === 0) {
              ...
              updateDoc(getGameRef(gameId), { turn: nextTurn, status: nextStatus, passEvent: { color: gameState.turn, time: Date.now() } }).catch(e => {});
           }
        }
      ...
      try {
        ...
        if (oppUid?.startsWith('ai_bot_')) {
           try {
             ...
             const n = new Date();
             const isRest = n.getHours() === 23 && n.getMinutes() >= 45;
             await updateDoc(getLeaderboardRef(oppUid), { rating: newAiRating, status: isRest ? 'offline' : 'online' }).catch(()=>{});
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    const isRestTime = now.getHours() === 23 && now.getMinutes() >= 45;
    if (isRestTime) {
      setToastMessage("AIプレイヤーは休憩中(23:45〜23:59)のため参戦できません。");
      setTimeout(() => setToastMessage(null), 3500);
      return;
    }
    ...
    try {
      ...
      await updateDoc(getLeaderboardRef(aiId), { status: 'playing' }).catch(()=>{});
    } catch (e) { 
      setPlayMode(null);
      setCurrentTab('home');
    }
  ...
  const handleMove = async (row, col, forceColor = null) => {
    ...
    if (playMode === 'local') {
      ...
      setGameState({ ...gameState, board: newBoard, turn: nextTurn, status: nextStatus, passEvent }); 
      await updateDoc(getGameRef(gameId), { board: JSON.stringify(newBoard), turn: nextTurn, status: nextStatus, passEvent }).catch(e => {});
    }
  };
  ...
  const handleLeaveGame = async () => {
    ...
    if (playMode === 'ranked' && gameState?.status === 'playing') {
      try { 
        ...
        const oppUid = gameState.player1 === user.uid ? gameState.player2 : gameState.player1;
        if (oppUid && oppUid.startsWith('ai_bot_')) {
            const n = new Date();
            const isRest = n.getHours() === 23 && n.getMinutes() >= 45;
            updateDoc(getLeaderboardRef(oppUid), { status: isRest ? 'offline' : 'online' }).catch(()=>{});
        }
      } catch (e) {}
    }
