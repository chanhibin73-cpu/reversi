...
const getMoveFromBoards = (prevBoard, currentBoard, turn) => {
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      if (prevBoard[r][c] === null && currentBoard[r][c] === turn) {
        return { r, c };
      }
    }
  }
...
const calcRatingChange = (myRate, oppRate, myStones, oppStones, isWin, isDraw, currentStreak = 0, isAI = false, oppCurrentStreak = 0) => {
  ...
  let bonus = isDraw ? 0 : stoneDiff * 0.5; 
  
  if (isWin && currentStreak >= 5) {
    let winBonus = 10 + (currentStreak - 5) * 2;
    if (winBonus > 50) winBonus = 50; 
    if (isAI) winBonus = winBonus / 10; 
    bonus += winBonus;
  }

  if (isWin && oppCurrentStreak >= 5) {
    let stopBonus = (oppCurrentStreak - 4) * 10;
    if (stopBonus > 80) stopBonus = 80;
    if (isAI) stopBonus = stopBonus / 10;
    bonus += stopBonus;
  }
...
const getAiDepth = (rating, isStrongAI = false) => {
  let depth = 3 + Math.floor((rating - 1000) / 400); 
  if (isStrongAI) {
...
const evaluateBoard = (board, color) => {
  ...
  if (emptyStones <= 20) score += (myStones - oppStones) * 5;
...
const minimax = (board, depth, alpha, beta, isMaximizing, color, origColor, endTime) => {
  ...
  if (moves.length === 0) {
    ...
    if (getSimulatedMoves(board, opp).length === 0) {
      ...
      return myCount > oppCount ? 1000 + (myCount - oppCount)*10 : (myCount < oppCount ? -1000 - (oppCount - myCount)*10 : 0);
    }
...
export default function OthelloApp() {
  ...
  const [profile, setProfile] = useState({ 
    ...
    wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], ratingHistory: [], friends: [], theme: 'green' 
  });
  ...
  const [currentAnalysisItem, setCurrentAnalysisItem] = useState(null);
  ...
  const triggerCommError = (msg = "通信エラーです。") => {
    setToastMessage(msg);
  ...
  const renderEvalGraph = () => {
    ...
    const validEvals = evalHistory.filter(v => v !== undefined && v !== null);
    let absMax = validEvals.length > 0 ? Math.max(...validEvals.map(Math.abs)) : 100;
    if (absMax < 100) absMax = 100;
    if (absMax > 500) absMax = 500;

    const points = evalHistory.map((val, i) => {
      if (val === undefined || val === null) return null;
      let clamped = Math.max(-absMax, Math.min(absMax, val));
      const x = (i / maxTurn) * w;
      const y = h/2 - (clamped / absMax) * (h/2);
      return `${x},${y}`;
    });

    const validPoints = points.filter(p => p !== null).join(' ');
    const turnIndex = playMode === 'history_analysis' ? analysisIndex : (gameState?.boardHistory?.length ? gameState.boardHistory.length - 1 : 0);
    ...
    return (
      <div className="w-full h-[60px] bg-gray-50 border border-gray-200 rounded-lg relative overflow-hidden shrink-0 mt-1">
        ...
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
          <polyline points={validPoints} fill="none" stroke="#3b82f6" strokeWidth="2" strokeLinejoin="round" />
          {evalHistory[turnIndex] !== undefined && evalHistory[turnIndex] !== null && (
            <circle cx={(turnIndex / maxTurn) * w} cy={h/2 - (Math.max(-absMax, Math.min(absMax, evalHistory[turnIndex])) / absMax) * (h/2)} r="3" fill="#ef4444" />
          )}
        </svg>
      </div>
    );
  ...
  useEffect(() => {
    const handlePageHide = () => {
      if (gameId && playMode === 'matching') {
        deleteDoc(getGameRef(gameId)).catch(() => {});
      }
    };
  ...
  useEffect(() => {
    ...
    if (myLbData?.invitation) {
      if (Date.now() - myLbData.invitation.time < 60000) { 
        if (!incomingInvitation || incomingInvitation.time !== myLbData.invitation.time) {
          setIncomingInvitation(myLbData.invitation);
        }
      } else {
         updateDoc(getLeaderboardRef(user.uid), { invitation: null }).catch(()=>{});
      }
    } else {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        try {
          ...
          const needsReset = aiCheck.exists() && aiCheck.data().resetVersion !== RESET_VERSION;
          if (!aiCheck.exists() || needsReset) {
            for (let i = 0; i < 20; i++) {
              const uid = `ai_bot_${i}`;
              const aiName = `AI-Player-${i+1}`;
              await setDoc(getLeaderboardRef(uid), { 
                uid, name: aiName, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, status: 'online', resetVersion: RESET_VERSION,
                highestRating: 1000, wins: 0, losses: 0, draws: 0, currentStreak: 0
              }).catch(()=>{});
              await setDoc(getProfileRef(uid), { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], ratingHistory: [], theme: 'dark', isAI: true, resetVersion: RESET_VERSION }).catch(()=>{});
            }
          }
        } catch(e) {}

        try {
          const strongAiCheck = await getDoc(getLeaderboardRef('ai_bot_strong_3'));
          const needsReset = strongAiCheck.exists() && strongAiCheck.data().resetVersion !== RESET_VERSION;
          if (!strongAiCheck.exists() || needsReset) {
            for (let i = 0; i < 4; i++) {
              const uid = `ai_bot_strong_${i}`;
              const aiName = `AI-Master-${i+1}`;
              await setDoc(getLeaderboardRef(uid), { 
                uid, name: aiName, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, isStrongAI: true, status: 'online', resetVersion: RESET_VERSION,
                highestRating: 1000, wins: 0, losses: 0, draws: 0, currentStreak: 0
              }).catch(()=>{});
              await setDoc(getProfileRef(uid), { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], ratingHistory: [], theme: 'dark', isAI: true, isStrongAI: true, resetVersion: RESET_VERSION }).catch(()=>{});
            }
          }
        } catch(e) {}
        ...
        try {
          ...
          if (pSnap.exists()) {
            ...
            if (pData.resetVersion !== RESET_VERSION) {
              ...
              await setDoc(getLeaderboardRef(currentUser.uid), { 
                uid: currentUser.uid, name: pData.name, rating: 1000, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, status: 'online', resetVersion: RESET_VERSION,
                highestRating: 1000, wins: 0, losses: 0, draws: 0, currentStreak: 0
              }, { merge: true });
            }
  ...
  const handleInitialRegistration = async () => {
    ...
    try {
      ...
      await setDoc(getLeaderboardRef(user.uid), { 
        uid: user.uid, name: newProfile.name, rating: newProfile.rating, dailyDate: todayStr, dailyRating: newProfile.rating, dailyRank: 0, status: 'online', resetVersion: RESET_VERSION,
        highestRating: selectedRate, wins: 0, losses: 0, draws: 0, currentStreak: 0
      });
      setProfile(newProfile);
      setIsFirstLogin(false);
    } catch (e) { 
      setInitError("通信エラーです。");
    }
  };
  ...
  const updateProfile = async (updates) => {
    ...
    if (user) {
      ...
      if (updates.rating !== undefined || updates.name !== undefined || updates.wins !== undefined || updates.history !== undefined) {
        await setDoc(getLeaderboardRef(user.uid), { 
          uid: user.uid, 
          name: newProfile.name, 
          rating: newProfile.rating,
          highestRating: newProfile.highestRating,
  ...
  const handleUserClick = async (targetUser) => {
    ...
    try {
      ...
      if (snap.exists()) {
        setSelectedFriend(prev => prev?.uid === targetUser.uid ? { ...targetUser, profile: snap.data() } : prev);
      }
    } catch (e) {
      triggerCommError();
    }
  ...
  const handleInvite = async (targetUid, mode) => {
    ...
    if (mode === 'room') {
      newRoomPin = Math.floor(1000 + Math.random() * 9000).toString(); 
      ...
      if (isAI) {
        ...
        try {
          const aiSnap = await getDoc(getLeaderboardRef(targetUid));
          if (aiSnap.exists()) aiData = { ...aiData, ...aiSnap.data() };
        } catch(e) {}
      }

      await setDoc(getGameRef(newRoomPin), {
        gameType: 'room', 
        status: isAI ? 'playing' : 'waiting', 
        board: JSON.stringify(initB), turn: 'black',
        boardHistory: [JSON.stringify(initB)], turnHistory: ['black'],
        player1: isP1 ? user.uid : (isAI ? targetUid : null), 
        player1Data: isP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : (isAI ? { name: aiData.name, rating: aiData.rating, currentStreak: aiData.currentStreak || 0 } : null), 
        player2: !isP1 ? user.uid : (isAI ? targetUid : null), 
        player2Data: !isP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : (isAI ? { name: aiData.name, rating: aiData.rating, currentStreak: aiData.currentStreak || 0 } : null), 
        createdAt: Date.now(),
        allowAiAnalysis: true 
      });
      newGameId = newRoomPin;
    } else if (mode === 'ranked') {
      const initB = INITIAL_BOARD.map(r => [...r]);
      const isCreatorP1 = Math.random() < 0.5;
      ...
      const newGameRef = await addDoc(getGamesCollection(), {
        ...
        board: JSON.stringify(initB), turn: 'black',
        boardHistory: [JSON.stringify(initB)], turnHistory: ['black'],
        player1: isCreatorP1 ? user.uid : (isAI ? targetUid : null), 
        player1Data: isCreatorP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : (isAI ? { name: aiData.name, rating: aiData.rating, currentStreak: aiData.currentStreak || 0 } : null), 
        player2: !isCreatorP1 ? user.uid : (isAI ? targetUid : null), 
        player2Data: !isCreatorP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : (isAI ? { name: aiData.name, rating: aiData.rating, currentStreak: aiData.currentStreak || 0 } : null), 
    ...
    if (!isAI) {
      await updateDoc(getLeaderboardRef(targetUid), {
        invitation: {
          fromUid: user.uid,
      ...
      triggerCommError(`招待を送信しました！`); 
    } else {
      triggerCommError(`AIとの対局を開始します。`);
    }
  ...
  const acceptInvitation = async () => {
    ...
    setIncomingInvitation(null);
    await updateDoc(getLeaderboardRef(user.uid), { invitation: null }).catch(()=>{});

    setToastMessage(null);
    setLastMatchChange(null);
    setShowRoomModal(false); 
    setPlayMode('matching'); 
    setCurrentTab('play');
    ...
    try {
        ...
        if (roomDoc.exists() && roomDoc.data().status === 'waiting') {
          ...
          if (!roomData.player1) {
            transaction.update(roomRef, { 
              status: 'playing', 
              player1: user.uid, 
              player1Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } 
            });
          } else {
            transaction.update(roomRef, { 
              status: 'playing', 
              player2: user.uid, 
              player2Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } 
            });
          }
          joined = true;
        }
      ...
      if (joined) {
        ...
        triggerCommError("部屋がキャンセルされたか、すでに開始されています。");
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    try {
      ...
      snap.forEach(d => {
        const data = d.data();
        if(data.uid?.startsWith('ai_bot_') && data.resetVersion === RESET_VERSION) aiList.push(data);
      });

      let suitableAIs = aiList.filter(ai => Math.abs(ai.rating - profile.rating) < 800);
      if (suitableAIs.length === 0) suitableAIs = aiList; 

      let selectedAI = suitableAIs[Math.floor(Math.random() * suitableAIs.length)];
      if (!selectedAI) {
         const isStrong = Math.random() < (4/24);
         const aiIndex = isStrong ? Math.floor(Math.random() * 4) : Math.floor(Math.random() * 20);
         selectedAI = { uid: isStrong ? `ai_bot_strong_${aiIndex}` : `ai_bot_${aiIndex}`, name: isStrong ? `AI-Master-${aiIndex+1}` : `AI-Player-${aiIndex+1}`, rating: 1000 };
      }

      const aiId = selectedAI.uid;
      let aiData = { name: selectedAI.name, rating: selectedAI.rating, highestRating: 1000, wins: 0, losses: 0, draws: 0, currentStreak: 0 };
      
      const aiProfileSnap = await getDoc(getProfileRef(aiId));
      if (aiProfileSnap.exists()) aiData = { ...aiData, ...aiProfileSnap.data() };

      const gameSnap = await getDoc(getGameRef(gid));
      const gameData = gameSnap.exists() ? gameSnap.data() : {};

      const isPlayerP1 = gameData.player1 === user.uid;
      
      await updateDoc(getGameRef(gid), { 
        status: 'playing', 
        player1: isPlayerP1 ? user.uid : aiId,
        player1Data: isPlayerP1 ? gameData.player1Data : { 
          name: aiData.name || `AI-Player`, 
          rating: aiData.rating || 1000, 
          highestRating: aiData.highestRating || 1000, 
          wins: aiData.wins||0, losses: aiData.losses||0, draws: aiData.draws||0, currentStreak: aiData.currentStreak || 0 
        },
        player2: isPlayerP1 ? aiId : user.uid, 
        player2Data: !isPlayerP1 ? gameData.player2Data : { 
          name: aiData.name || `AI-Player`, 
          rating: aiData.rating || 1000, 
          highestRating: aiData.highestRating || 1000, 
          wins: aiData.wins||0, losses: aiData.losses||0, draws: aiData.draws||0, currentStreak: aiData.currentStreak || 0 
  ...
  const startRankedMatch = async () => {
    ...
    try {
      ...
      snap.forEach(doc => {
        ...
        if (data.status === 'waiting' && data.gameType === 'ranked') {
          if (data.createdAt && (now - data.createdAt > 60000)) {
            deleteDoc(getGameRef(doc.id)).catch(() => {});
          } else {
            const oppRating = data.player1Data ? data.player1Data.rating : (data.player2Data ? data.player2Data.rating : 1000);
            const oppUid = data.player1 || data.player2;
            if (oppUid && oppUid !== user.uid) {
              if (Math.abs(oppRating - profile.rating) < 800) {
                foundGame = { id: doc.id, ...data };
              } else {
                backupGame = { id: doc.id, ...data };
              }
            }
          }
        }
      ...
      if (!foundGame && backupGame && Math.random() < 0.1) {
         foundGame = backupGame;
      }
      ...
      if (foundGame) {
        try {
          await runTransaction(db, async (transaction) => {
            const gameDoc = await transaction.get(getGameRef(foundGame.id));
            if (gameDoc.exists() && gameDoc.data().status === 'waiting') {
              const gameData = gameDoc.data();
              if (!gameData.player1) {
                transaction.update(getGameRef(foundGame.id), { 
                  status: 'playing', 
                  player1: user.uid, 
                  player1Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } 
                });
      ...
      if (joined) {
        ...
        const newGameRef = await addDoc(getGamesCollection(), {
          ...
          boardHistory: [JSON.stringify(initB)], turnHistory: ['black'],
          player1: isP1 ? user.uid : null, 
          player1Data: isP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : null, 
          player2: !isP1 ? user.uid : null, 
          player2Data: !isP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : null, 
          resultProcessedBy: [], createdAt: Date.now()
        });
  ...
  const joinRoom = async (roomPin) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!roomDoc.exists()) {
          ...
          transaction.set(roomRef, { 
            gameType: 'room', status: 'waiting', board: JSON.stringify(initB), turn: 'black', 
            boardHistory: [JSON.stringify(initB)], turnHistory: ['black'],
            player1: isP1 ? user.uid : null, 
            player1Data: isP1 ? { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } : null, 
            player2: !isP1 ? user.uid : null, 
          ...
          if (data.status === 'waiting' && (!data.player1 || !data.player2)) {
             if (!data.player1 && data.player2 !== user.uid) {
               transaction.update(roomRef, { status: 'playing', player1: user.uid, player1Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } }); role = 'player';
             } else if (!data.player2 && data.player1 !== user.uid) {
               transaction.update(roomRef, { status: 'playing', player2: user.uid, player2Data: { name: profile.name, rating: profile.rating, currentStreak: profile.currentStreak || 0 } }); role = 'player';
             }
      ...
      triggerCommError("入室時に通信エラーが発生しました。");
      setPlayMode(null); setCurrentTab('home');
    }
  };
  ...
  const handleMove = async (row, col, forceColor = null) => {
    ...
    if (!myColor) {
      myColor = (playMode === 'local' || playMode === 'research') ? gameState.turn : (gameState.player1 === user.uid ? 'black' : 'white');
    ...
    let newBoardHistory = gameState.boardHistory || [JSON.stringify(INITIAL_BOARD)];
    newBoardHistory = [...newBoardHistory, JSON.stringify(newBoard)];
    let newTurnHistory = gameState.turnHistory || ['black'];
    newTurnHistory = [...newTurnHistory, nextTurn];

    if (playMode === 'local' || playMode === 'research') {
      setLocalHistory([...localHistory, newBoard]);
      setGameState({ ...gameState, board: newBoard, turn: nextTurn, status: nextStatus, passEvent, boardHistory: newBoardHistory, turnHistory: newTurnHistory });
    } else {
      setGameState({ ...gameState, board: newBoard, turn: nextTurn, status: nextStatus, passEvent, boardHistory: newBoardHistory, turnHistory: newTurnHistory }); 
      await updateDoc(getGameRef(gameId), { board: JSON.stringify(newBoard), boardHistory: newBoardHistory, turnHistory: newTurnHistory, turn: nextTurn, status: nextStatus, passEvent }).catch(e => {});
    }
  };
  ...
  const handleStamp = async (text) => {
    ...
    await updateDoc(getGameRef(gameId), { stamp: { sender: user.uid, text, time: Date.now() } }).catch(e => {});
    ...
    if (gameId && playMode === 'ranked') {
      if (gameState?.status === 'waiting' || !gameState) {
        ...
        try { 
          ...
          const myProfileSnap = await getDoc(getProfileRef(user.uid));
          const currentProfile = myProfileSnap.exists() ? myProfileSnap.data() : profile;

          const change = calcRatingChange(currentProfile.rating || 1000, oppData?.rating || 1000, 0, 64, false, false, currentProfile.currentStreak || 0, false, oppData?.currentStreak || 0);
          const newRating = Math.max(0, (currentProfile.rating || 1000) + change);
          const historyItem = { 
            id: Date.now(), oppName: oppData?.name || '対戦相手', result: 'loss', change, myStones: 0, oppStones: 64, 
          ...
          await updateProfile({
            rating: newRating, 
            currentStreak: 0, 
            losses: (currentProfile.losses || 0) + 1, 
    ...
    const interval = setInterval(async () => {
      try {
        if (Math.random() < 0.60) { 
          ...
          snap.forEach(d => { 
            const data = d.data();
            if(data.uid?.startsWith('ai_bot_') && data.status === 'online' && data.resetVersion === RESET_VERSION) aiList.push(data); 
          });

          for (let pair = 0; pair < 2; pair++) {
            if (aiList.length >= 2) {
              const idx1 = Math.floor(Math.random() * aiList.length);
              const a1 = aiList[idx1];
              aiList.splice(idx1, 1);

              let a2 = null;
              let suitableOpponents = aiList.filter(ai => Math.abs(ai.rating - a1.rating) < 800);
              if (suitableOpponents.length > 0) {
                 const idx2 = Math.floor(Math.random() * suitableOpponents.length);
                 a2 = suitableOpponents[idx2];
                 aiList = aiList.filter(ai => ai.uid !== a2.uid);
              } else {
                 const idx2 = Math.floor(Math.random() * aiList.length);
                 a2 = aiList[idx2];
                 aiList.splice(idx2, 1);
              }
              
              if (!a2) continue;

              if (Math.abs(a1.rating - a2.rating) >= 800 && Math.random() < 0.8) continue;
              ...
              setTimeout(async () => {
                let p1Data = { rating: a1.rating, currentStreak: 0, highestRating: 1000, wins: 0, losses: 0, draws: 0 };
                let p2Data = { rating: a2.rating, currentStreak: 0, highestRating: 1000, wins: 0, losses: 0, draws: 0 };
                try {
                  const [snap1, snap2] = await Promise.all([getDoc(getProfileRef(a1.uid)), getDoc(getProfileRef(a2.uid))]);
                  if (snap1.exists()) p1Data = { ...p1Data, ...snap1.data() };
                  if (snap2.exists()) p2Data = { ...p2Data, ...snap2.data() };
                } catch(e) {}

                const expected = 1 / (1 + Math.pow(10, (p2Data.rating - p1Data.rating) / 400));
                ...
                const a1Change = calcRatingChange(p1Data.rating, p2Data.rating, myStones, oppStones, isA1Win && !isDraw, isDraw, p1Data.currentStreak, true, p2Data.currentStreak);
                const a2Change = calcRatingChange(p2Data.rating, p1Data.rating, oppStones, myStones, !isA1Win && !isDraw, isDraw, p2Data.currentStreak, true, p1Data.currentStreak);
                
                const updates = [
                  {uid:a1.uid, change:a1Change, isWin:isA1Win&&!isDraw, isDraw, pData: p1Data}, 
                  {uid:a2.uid, change:a2Change, isWin:!isA1Win&&!isDraw, isDraw, pData: p2Data}
                ];

                for (let target of updates) {
                   const newRating = Math.max(0, (target.pData.rating || 1000) + target.change);
                   const newHighestRating = Math.max(target.pData.highestRating || 1000, newRating);
                   const newCurrentStreak = target.isWin ? (target.pData.currentStreak || 0) + 1 : 0;
                   const newWins = (target.pData.wins || 0) + (target.isWin ? 1 : 0);
                   const newLosses = (target.pData.losses || 0) + (!target.isWin && !target.isDraw ? 1 : 0);
                   const newDraws = (target.pData.draws || 0) + (target.isDraw ? 1 : 0);

                   await updateDoc(getLeaderboardRef(target.uid), { 
                     rating: newRating, status: 'online',
                   ...
                   try {
                     await updateDoc(getProfileRef(target.uid), {
                       rating: newRating, highestRating: newHighestRating, currentStreak: newCurrentStreak, wins: newWins, losses: newLosses, draws: newDraws
                     });
                   } catch(e) {} 
                }
              }, 4000); 
  ...
  const startResearchMode = () => {
    ...
    setLastMatchChange(null);
    const initB = INITIAL_BOARD.map(r => [...r]);
    setGameState({ status: 'playing', board: initB, turn: 'black', player1Data: { name: 'Player(黒)' }, player2Data: { name: 'Player(白)' } });
    setLocalHistory([initB]); 
    setPlayMode('research'); 
    setCurrentTab('play');
  ...
  const startHistoryAnalysis = (item) => {
    if (!item.analysisData) return;
    setCurrentAnalysisItem(item);
    
    const parsedHistory = item.boardHistory.map(b => JSON.parse(b));
    setAnalysisHistory(parsedHistory);
    setAnalysisTurnHistory(item.turnHistory || parsedHistory.map((_, i) => i % 2 === 0 ? 'black' : 'white'));
    setAnalysisIndex(0);
    
    const initialTurn = item.turnHistory ? item.turnHistory[0] : 'black';
    setGameState({ 
      status: 'playing', 
      board: parsedHistory[0], 
      turn: initialTurn, 
      player1Data: { name: '黒(先手)' }, 
      player2Data: { name: '白(後手)' },
      boardHistory: item.boardHistory,
      turnHistory: item.turnHistory
    });
    
    setPlayMode('history_analysis');
    setCurrentTab('play');

    const evalForBlack = item.analysisData.evalHistory[0];
    const evalForTurn = initialTurn === 'black' ? evalForBlack : -evalForBlack;

    setResearchData({ 
      topMoves: item.analysisData.topMovesHistory[0] || [], 
      evaluation: evalForTurn, 
      isAnalyzing: false, 
      actualMoveEval: item.analysisData.actualMoveEvals[0] 
    });
    setIsDeepAnalysis(item.analysisData.depthMode === 'deep');
    setEvalHistory(item.analysisData.evalHistory);
  };

  const changeAnalysisIndex = (newIndex) => {
    setAnalysisIndex(newIndex);
    const currentTurn = analysisTurnHistory[newIndex] || 'black';
    setGameState(prev => ({
      ...prev,
      board: analysisHistory[newIndex],
      turn: currentTurn
    }));

    if (currentAnalysisItem?.analysisData) {
       const evalForBlack = currentAnalysisItem.analysisData.evalHistory[newIndex];
       const evalForTurn = currentTurn === 'black' ? evalForBlack : -evalForBlack;

       setResearchData({
         topMoves: currentAnalysisItem.analysisData.topMovesHistory[newIndex] || [],
         evaluation: evalForTurn,
         isAnalyzing: false,
         actualMoveEval: currentAnalysisItem.analysisData.actualMoveEvals[newIndex]
       });
    }
  };

  const runFullAnalysis = (item, mode) => {
    setShowAnalysisModal(null);
    const parsedBoards = item.boardHistory.map(b => JSON.parse(b));
    const turns = item.turnHistory || parsedBoards.map((_, i) => i % 2 === 0 ? 'black' : 'white');
    
    setAnalysisProgress({ current: 0, total: parsedBoards.length, mode });

    let newEvalHistory = [];
    let newActualMoveEvals = [];
    let newTopMovesHistory = [];

    const analyzeTurn = (idx) => {
      if (idx >= parsedBoards.length) {
        const newData = { evalHistory: newEvalHistory, actualMoveEvals: newActualMoveEvals, topMovesHistory: newTopMovesHistory, depthMode: mode };
        const newHistory = profile.history.map(h => h.id === item.id ? { ...h, analysisData: newData } : h);
        updateProfile({ history: newHistory }).then(() => {
          setAnalysisProgress(null);
          const updatedItem = newHistory.find(h => h.id === item.id);
          startHistoryAnalysis(updatedItem);
        });
        return;
      }

      setAnalysisProgress({ current: idx, total: parsedBoards.length, mode });

      setTimeout(() => {
        const board = parsedBoards[idx];
        const turn = turns[idx];

        let emptyCount = 0;
        board.forEach(r => r.forEach(c => { if (c === null) emptyCount++; }));
        
        let depth = mode === 'deep' ? 15 : 5;
        if (mode === 'deep' && emptyCount <= 15) depth = emptyCount;
        else if (mode === 'normal' && emptyCount <= 8) depth = emptyCount;

        const moves = getSimulatedMoves(board, turn);
        if (moves.length === 0) {
          const { black, white } = countStones(board);
          const myCount = turn === 'black' ? black : white;
          const oppCount = turn === 'black' ? white : black;
          let evaluation = myCount > oppCount ? 500 : (myCount < oppCount ? -500 : 0);
          newEvalHistory.push(turn === 'black' ? evaluation : -evaluation);
          newActualMoveEvals.push(null);
          newTopMovesHistory.push([]);
          analyzeTurn(idx + 1);
          return;
        }

        let evaluatedMoves = [];
        let bestEvaluatedMoves = [];
        const endTime = mode === 'deep' ? Date.now() + 15000 : Date.now() + 3000;

        try {
          const startDepth = mode === 'deep' ? 4 : 4;
          for (let d = startDepth; d <= depth; d++) {
            let tempEvals = [];
            for (let move of moves) {
              let newBoard = board.map(r => [...r]);
              newBoard[move.r][move.c] = turn;
              move.flips.forEach(([fr, fc]) => newBoard[fr][fc] = turn);
              let score = minimax(newBoard, d - 1, -Infinity, Infinity, false, turn === 'black' ? 'white' : 'black', turn, endTime);
              score = clampScore(score);
              tempEvals.push({ ...move, score });
            }
            tempEvals.sort((a, b) => b.score - a.score);
            bestEvaluatedMoves = tempEvals;
            if (bestEvaluatedMoves[0].score === 500 || bestEvaluatedMoves[0].score === -500) {
          ...
          if (evaluatedMoves.length === 0) {
             evaluatedMoves = moves.map(move => {
                let newBoard = board.map(r => [...r]);
                newBoard[move.r][move.c] = turn;
                move.flips.forEach(([fr, fc]) => newBoard[fr][fc] = turn);
                let score = evaluateBoard(newBoard, turn);
                return { ...move, score };
             });
        ...
        newEvalHistory.push(turn === 'black' ? evaluation : -evaluation);
        newTopMovesHistory.push(evaluatedMoves.slice(0, 3));

        let actualMoveEval = null;
        if (idx < parsedBoards.length - 1) {
          const actualMove = getMoveFromBoards(board, parsedBoards[idx + 1], turn);
          if (actualMove) {
            const matched = evaluatedMoves.find(m => m.r === actualMove.r && m.c === actualMove.c);
            if (matched) {
              const diff = evaluation - matched.score;
              let judgment = '最善手';
              if (diff > 0) judgment = '好手';
              if (diff > 5) judgment = '疑問手';
              if (diff > 15) judgment = '悪手';
              if (diff === 0) judgment = '最善手';
              actualMoveEval = { ...matched, judgment, diff };
            } else {
              let newBoard = board.map(r => [...r]);
              newBoard[actualMove.r][actualMove.c] = turn;
              const flips = getFlippableStones(board, actualMove.r, actualMove.c, turn);
              flips.forEach(([fr, fc]) => newBoard[fr][fc] = turn);
              let score = minimax(newBoard, 3, -Infinity, Infinity, false, turn === 'black' ? 'white' : 'black', turn);
              score = clampScore(score);
              const diff = evaluation - score;
              let judgment = '大悪手';
              if (diff <= 5) judgment = '疑問手';
              else if (diff <= 15) judgment = '悪手';
              actualMoveEval = { r: actualMove.r, c: actualMove.c, score, judgment, diff };
            }
          }
        }
        newActualMoveEvals.push(actualMoveEval);
  ...
  useEffect(() => {
    const isRealTimeAnalysis = playMode === 'research' || (playMode === 'room' && gameState?.allowAiAnalysis);
    if (isRealTimeAnalysis && gameState && gameState.status === 'playing') {
      ...
      const timerId = setTimeout(() => {
        ...
        let depth = isDeepAnalysis ? 15 : 5; 
        if (!isDeepAnalysis && emptyCount <= 10) depth = emptyCount;

        const moves = getSimulatedMoves(gameState.board, gameState.turn);
        if (moves.length === 0) {
          const { black, white } = countStones(gameState.board);
          const myCount = gameState.turn === 'black' ? black : white;
          const oppCount = gameState.turn === 'black' ? white : black;
          let evaluation = myCount > oppCount ? 500 : (myCount < oppCount ? -500 : 0);

          setResearchData({ topMoves: [], evaluation, isAnalyzing: false, actualMoveEval: null });
          const turnIndex = localHistory.length > 0 ? localHistory.length - 1 : (gameState.boardHistory?.length ? gameState.boardHistory.length - 1 : 0);
          
          const evalForBlack = gameState.turn === 'black' ? evaluation : -evaluation;
          setEvalHistory(prev => {
            const newHistory = [...prev];
            newHistory[turnIndex] = clampScore(evalForBlack);
            return newHistory;
          });
          return;
        }
        ...
        let evaluatedMoves = [];
        let bestEvaluatedMoves = [];
        const endTime = isDeepAnalysis ? Date.now() + 15000 : Date.now() + 3000;

        try {
          const startDepth = isDeepAnalysis ? 4 : 4;
          for (let d = startDepth; d <= depth; d++) {
            let tempEvals = [];
            for (let move of moves) {
              let newBoard = gameState.board.map(r => [...r]);
          ...
          if (evaluatedMoves.length === 0) {
             evaluatedMoves = moves.map(move => {
                ...
                newBoard[move.r][move.c] = gameState.turn;
                move.flips.forEach(([fr, fc]) => newBoard[fr][fc] = gameState.turn);
                let score = evaluateBoard(newBoard, gameState.turn);
                return { ...move, score };
             });
             evaluatedMoves.sort((a, b) => b.score - a.score);
          }
        ...
        let evaluation = evaluatedMoves.length > 0 ? evaluatedMoves[0].score : 0;
        setResearchData({ topMoves: evaluatedMoves.slice(0, 3), evaluation, isAnalyzing: false, actualMoveEval: null });
  ...
  }, [gameState?.board, gameState?.turn, playMode, isDeepAnalysis, gameState?.allowAiAnalysis]);

  // --- AI Turn Execution ---
  useEffect(() => {
    if (!gameState || gameState.status !== 'playing') return;
    if (playMode !== 'ranked' && playMode !== 'room') return;
    ...
    if (aiId?.startsWith('ai_bot_') && (gameState.player1 === user?.uid || gameState.player2 === user?.uid)) {
      ...
      const isStrongAI = aiId.includes('_strong_');
      const referenceRating = aiData?.rating || 1000;
      const depth = getAiDepth(referenceRating, isStrongAI);
        ...
        if (moveStr) {
          ...
          handleMove(r, c, gameState.turn);
           ...
           if (getValidMoves(gameState.board, gameState.turn).length === 0) {
              ...
              updateDoc(getGameRef(gameId), { turn: nextTurn, status: nextStatus, passEvent: { color: gameState.turn, time: Date.now() } }).catch(e => { triggerCommError(); });
           }
        }
      }, 10);
  ...
  const renderHome = () => (
    <div className="p-4 space-y-5 flex flex-col min-h-full animate-in fade-in duration-300">
      ...
      <div className="space-y-3 flex-1">
        ...
        <button onClick={startResearchMode} className="w-full bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between active:scale-95 transition-transform">
          <div className="flex items-center space-x-4"><div className={`p-3 rounded-full ${activeTheme.light} ${activeTheme.text}`}><Bot size={24} /></div><div className="text-left"><h3 className="font-bold text-gray-800">AI研究モード</h3><p className="text-xs text-gray-500">AIが最善手をリアルタイム解析</p></div></div><ChevronRight size={20} className="text-gray-400" />
        </button>
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-xs space-y-4">
            ...
            <div className="flex items-center justify-center mt-2">
              <input type="checkbox" id="allowAi" checked={allowAiAnalysis} onChange={(e) => setAllowAiAnalysis(e.target.checked)} className="mr-2" />
              <label htmlFor="allowAi" className="text-xs font-bold text-gray-600 cursor-pointer select-none">AI解析を許可する（ホストのみ）</label>
            </div>
              ...
              {leaderboardData.slice(0, 30).map((lbUser, idx) => {
                ...
                return (
                  <div key={lbUser.uid} onClick={() => handleUserClick(lbUser)} className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer hover:bg-gray-50 transition-colors ${lbUser.uid === user?.uid ? 'bg-blue-50 border-blue-200 shadow-sm ring-1 ring-blue-200 hover:bg-blue-50' : 'bg-white border-gray-100 shadow-sm'}`}>
                    ...
                    {lbUser.uid !== user?.uid && (
                      <button onClick={(e) => { e.stopPropagation(); toggleFriend(lbUser.uid); }} className="ml-2 p-1.5 rounded-full hover:bg-gray-200 transition-colors">
            ...
            friendList.map((fUser) => (
              <div key={fUser.uid} onClick={() => handleUserClick(fUser)} className="flex items-center justify-between p-4 rounded-xl bg-white border border-gray-100 shadow-sm cursor-pointer hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); toggleFriend(fUser.uid); }} className="p-2 bg-gray-50 rounded-full text-gray-400 hover:text-red-500 transition-colors"><UserMinus size={16} /></button>
                </div>
              </div>
            ))
          )}
  ...
  const renderGame = () => {
    if (playMode === 'matching') return (
      <div className="h-full flex flex-col items-center justify-center space-y-4 px-6 text-center">
        ...
        <p className="text-xs text-gray-400">※20秒経過するとAIプレイヤーと対戦が開始されます。</p>
        {gameId && (
      ...
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        <div className="flex items-center justify-between mb-4 shrink-0">
           {playMode !== 'history_analysis' ? (
             ...
             <button onClick={() => { setPlayMode(null); setCurrentTab('profile'); }} className="p-2 bg-white rounded-full shadow-sm text-gray-500 hover:text-gray-800"><ArrowLeft size={20} /></button>
           )}
          ...
          <div className="bg-white p-3 rounded-xl shadow-sm mb-4 border border-blue-100 flex flex-col shrink-0" style={{ height: playMode === 'history_analysis' ? '216px' : '172px' }}>
            <div className="flex justify-between items-center mb-2 shrink-0 h-6">
              <div className="flex items-center space-x-2">
                ...
                {playMode !== 'history_analysis' && (
                  <button onClick={() => setIsDeepAnalysis(!isDeepAnalysis)} className={`text-[10px] px-2 py-0.5 rounded font-bold transition-colors ${isDeepAnalysis ? 'bg-indigo-500 text-white shadow-inner' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>超解析</button>
                )}
              </div>
              <span className={`text-sm font-black ${researchData.isAnalyzing ? 'text-gray-500' : (turn === 'black' ? researchData.evaluation : -researchData.evaluation) > 0 ? 'text-gray-800' : (turn === 'black' ? researchData.evaluation : -researchData.evaluation) < 0 ? 'text-gray-400' : 'text-gray-500'}`}>
                形勢: {getEvaluationText()}
              </span>
            </div>
            ...
            <div className="flex space-x-2 h-[48px] shrink-0 mt-3">
              {researchData.topMoves.map((m, i) => {
                let dispScore = clampScore(m.score);
                if (gameState?.turn === 'white') dispScore = -dispScore;
                return (
                  <div key={i} className={`flex-1 text-center py-1.5 rounded-lg text-xs font-bold shadow-sm border ${i === 0 ? 'bg-red-50 text-red-600 border-red-200' : i === 1 ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                    {i + 1}位: {String.fromCharCode(65 + m.c)}{m.r + 1} <br/><span className="text-[10px] opacity-70">({dispScore > 0 ? '+' : ''}{dispScore})</span>
                  </div>
                );
              })}
              {researchData.topMoves.length === 0 && !researchData.isAnalyzing && (
                 <div className="w-full text-center py-2 text-xs text-gray-400 font-bold">候補手なし</div>
              )}
            </div>
            ...
            {playMode === 'history_analysis' && (
              <div className="h-[36px] mt-2 shrink-0">
                {researchData.actualMoveEval ? (() => {
                  let dispScore = clampScore(researchData.actualMoveEval.score);
                  if (gameState?.turn === 'white') dispScore = -dispScore;
                  return (
                    <div className={`p-2 rounded-lg text-center text-xs font-bold h-full flex items-center justify-center ${
                      researchData.actualMoveEval.judgment === '最善手' ? 'bg-blue-100 text-blue-700' :
                      researchData.actualMoveEval.judgment === '好手' ? 'bg-emerald-100 text-emerald-700' :
                      researchData.actualMoveEval.judgment === '疑問手' ? 'bg-yellow-100 text-yellow-700' :
                      'bg-red-100 text-red-700'
                    }`}>
                      実際の指し手: {String.fromCharCode(65 + researchData.actualMoveEval.c)}{researchData.actualMoveEval.r + 1} 
                      <span className="opacity-70 ml-2">({dispScore > 0 ? '+' : ''}{dispScore} / {researchData.actualMoveEval.judgment})</span>
                    </div>
                  );
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm mb-6 relative shrink-0">
          <div className="absolute top-0 left-0 w-full h-1 bg-gray-100 rounded-t-xl overflow-hidden"><div className={`h-full transition-all duration-500 ${gameState.turn === 'black' ? 'bg-black w-1/2' : 'bg-white w-1/2 ml-auto'}`}></div></div>
          
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${gameState.turn === 'black' ? activeTheme.light : ''} w-[100px]`}>
            {activeStamp && gameState.player1 && activeStamp.sender === gameState.player1 && (
            ...
            <div className="w-8 h-8 bg-black rounded-full shadow-md mb-1 border-2 border-transparent flex items-center justify-center">
              {gameState.player1?.startsWith('ai_bot_') && <span className="text-[10px] text-white font-mono opacity-50">AI</span>}
            </div>
            <span className="text-xs font-bold truncate w-full text-center">{gameState.player1Data?.name}</span>
            {(playMode === 'ranked' || playMode === 'room_spectator') && gameState.player1Data && (
              <span className="text-[10px] text-gray-500 font-bold w-full text-center whitespace-nowrap">
                R:{gameState.player1Data?.rating || 1000}{gameState.player1Data?.currentStreak >= 2 ? ` 🔥${gameState.player1Data.currentStreak}` : ''}
              </span>
            )}
            <span className="text-xs font-black mt-1">{countStones(gameState.board).black}</span>
          </div>

          <div className="text-center">
            {gameState.status === 'waiting' && <span className="text-xs font-bold text-gray-400">参加者を待機中...</span>}
            {gameState.status === 'playing' && <span className={`text-sm font-bold ${activeTheme.text}`}>{playMode === 'room_spectator' || playMode === 'history_analysis' ? (gameState.turn === 'black' ? '黒の番' : '白の番') : ((playMode === 'local' || playMode === 'research') ? gameState.turn : (gameState.player1 === user?.uid ? 'black' : 'white')) === gameState.turn ? "あなたの番です" : "相手の番です"}</span>}
            {gameState.status === 'finished' && <span className="text-sm font-bold text-red-500">決着！</span>}
          </div>

          <div className={`flex flex-col items-center p-2 rounded-lg relative ${gameState.turn === 'white' ? activeTheme.light : ''} w-[100px]`}>
            {activeStamp && gameState.player2 && activeStamp.sender === gameState.player2 && (
              <div className="absolute -top-10 bg-white px-3 py-1.5 rounded-2xl shadow-lg border border-gray-100 text-xs font-bold z-20 whitespace-nowrap animate-in zoom-in slide-in-from-bottom-2 duration-200">
                {activeStamp.text}<div className="absolute -bottom-1.5 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-white border-b border-r border-gray-100 rotate-45"></div>
              </div>
            )}
            <div className="w-8 h-8 bg-white rounded-full shadow-md mb-1 border-2 border-gray-200 flex items-center justify-center">
              {gameState.player2?.startsWith('ai_bot_') && <span className="text-[10px] text-black font-mono opacity-50">AI</span>}
            </div>
            <span className="text-xs font-bold truncate w-full text-center">{gameState.player2Data?.name || '---'}</span>
            {(playMode === 'ranked' || playMode === 'room_spectator') && gameState.player2Data && (
              <span className="text-[10px] text-gray-500 font-bold w-full text-center whitespace-nowrap">
                R:{gameState.player2Data?.rating || 1000}{gameState.player2Data?.currentStreak >= 2 ? ` 🔥${gameState.player2Data.currentStreak}` : ''}
              </span>
            )}
            <span className="text-xs font-black mt-1">{countStones(gameState.board).white}</span>
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center shrink-0">
          <div className={`${activeTheme.board} p-1.5 rounded-lg shadow-2xl w-full max-w-[350px] aspect-square`}>
            <div className={`grid grid-cols-8 grid-rows-8 gap-0.5 h-full w-full ${activeTheme.grid} border-2 ${activeTheme.border}`}>
              {gameState.board.map((row, rIdx) => row.map((cell, cIdx) => {
                const isMyTurn = playMode === 'room_spectator' || playMode === 'history_analysis' ? false : gameState.turn === ((playMode === 'local' || playMode === 'research') ? gameState.turn : (gameState.player1 === user?.uid ? 'black' : 'white'));
                const isMoveable = isMyTurn && gameState.status === 'playing' ? getValidMoves(gameState.board, gameState.turn).includes(`${rIdx},${cIdx}`) : false;
                let rankIndex = -1;
                if (isAnalysisMode && !researchData.isAnalyzing) {
                  rankIndex = researchData.topMoves.findIndex(m => m.r === rIdx && m.c === cIdx);
                }
                return (
                  <div key={`${rIdx}-${cIdx}`} onClick={() => isMoveable && handleMove(rIdx, cIdx)} className={`${activeTheme.cell} w-full h-full flex items-center justify-center relative ${isMoveable ? 'cursor-pointer hover:brightness-110' : ''}`}>
                    ...
                    {isMoveable && rankIndex === -1 && <div className="w-2.5 h-2.5 bg-black opacity-20 rounded-full"></div>}
                    {rankIndex !== -1 && (
                      <div className={`absolute top-0 right-0 w-3.5 h-3.5 rounded-bl-sm text-[8px] flex items-center justify-center font-black text-white ${rankIndex === 0 ? 'bg-red-500' : rankIndex === 1 ? 'bg-orange-500' : 'bg-blue-500'}`}>
                        {rankIndex + 1}
                      </div>
                    )}
                  </div>
        ...
        {playMode !== 'history_analysis' && (
          <div className="mt-6 flex space-x-3 relative shrink-0">
            {(playMode === 'ranked' || playMode === 'room') && gameState.status === 'playing' && (
            ...
            {playMode !== 'room_spectator' && (<button onClick={() => { if(playMode === 'local' || playMode === 'research') setGameState({...gameState, status:'finished', resignedBy:gameState.turn}); else updateDoc(getGameRef(gameId), { status:'finished', resignedBy:user.uid }); }} disabled={gameState.status !== 'playing'} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl font-bold active:scale-95 disabled:opacity-50">投了</button>)}
            {(playMode === 'local' || playMode === 'research') && (<button onClick={() => { const h = localHistory.slice(0, -1); setLocalHistory(h); setGameState({ ...gameState, board: h[h.length - 1], turn: gameState.turn === 'black' ? 'white' : 'black', status: 'playing', passEvent: null }); }} disabled={localHistory.length <= 1 || (gameState.status !== 'playing' && playMode !== 'research')} className={`flex-1 text-white py-3 rounded-xl font-bold active:scale-95 disabled:opacity-50 ${activeTheme.primary}`}>待った</button>)}
          </div>
        )}
        ...
        {gameState.status === 'finished' && !isAnalysisMode && (() => {
          const { black, white } = countStones(gameState.board);
          let winner = null;
          if (gameState.resignedBy) winner = gameState.resignedBy === gameState.player1 ? 'white' : 'black';
          else if (black > white) winner = 'black';
          else if (white > black) winner = 'white';
          else winner = 'draw';
          const myColor = (playMode === 'local' || playMode === 'research' || playMode === 'history_analysis') ? gameState.turn : (gameState.player1 === user?.uid ? 'black' : 'white');
          ...
          return (
            <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-40 animate-in fade-in">
              <div className="bg-white p-6 rounded-2xl shadow-xl max-w-sm w-full text-center space-y-4">
                ...
                {playMode === 'ranked' && lastMatchChange !== null && !gameState.resignedBy && (
                  <div className={`text-xl font-black ${lastMatchChange >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                    Rate: {lastMatchChange >= 0 ? `+${lastMatchChange}` : lastMatchChange}
                  </div>
                )}

                <div className="flex justify-center space-x-6 text-xl font-bold py-2"><span className={winner === 'black' ? activeTheme.text : 'text-gray-500'}>黒: {black}</span><span className="text-gray-300">-</span><span className={winner === 'white' ? activeTheme.text : 'text-gray-500'}>白: {white}</span></div>
  ...
  const renderProfileGraph = (history, key, isReverse = false) => {
    const data = [...(history || [])];
    if (data.length === 0 || data[data.length - 1].date !== todayStr) {
      data.push({ date: todayStr, rating: profile.rating, rank: currentRank || 0 });
    }
    const displayData = data.slice(-10);
    
    if (displayData.length < 2) {
      return <div className="text-xs text-gray-400 text-center py-6 font-bold">グラフを描画するためのデータが蓄積されていません。(明日以降に表示されます)</div>;
    }

    const values = displayData.map(d => d[key]);
    let max = Math.max(...values);
    let min = Math.min(...values);
    if (max - min < 20) {
      const mid = (max + min) / 2;
      max = Math.ceil(mid + 10);
      min = Math.floor(Math.max(0, mid - 10));
    }
    ...
    const points = displayData.map((d, i) => {
      const x = paddingX + (i / (displayData.length - 1)) * (w - paddingX * 2);
      let yVal = (d[key] - min) / range;
      if (isReverse) yVal = 1 - yVal;
      const y = paddingY + (1 - yVal) * (h - paddingY * 2);
      return `${x},${y}`;
    }).join(' ');
          ...
          {displayData.map((d, i) => {
            ...
            return (
              <g key={i}>
                 ...
                 <text x={x} y={y - 6} fontSize="9" fill="#6b7280" textAnchor="middle" fontWeight="black">{d[key]}{isReverse ? '位' : ''}</text>
                 <text x={x} y={h + 16} fontSize="7" fill="#9ca3af" textAnchor="middle">{formatDate(d.date)}</text>
              </g>
            )
          })}
        </svg>
    ...
    return (
      <div className="p-4 space-y-5 min-h-full animate-in fade-in duration-300 pb-8">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-2">
          <h3 className="font-bold text-gray-800 text-sm flex items-center"><BarChart2 size={16} className="mr-1.5 text-emerald-500" /> レーティング推移 (過去10日)</h3>
          {renderProfileGraph(profile.ratingHistory, 'rating', false)}
        </div>
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-2">
          <h3 className="font-bold text-gray-800 text-sm flex items-center"><BarChart2 size={16} className="mr-1.5 text-emerald-500" /> 順位推移 (過去10日)</h3>
          {renderProfileGraph(profile.ratingHistory, 'rank', true)}
        </div>
            ...
            <div className="space-y-4">
              {profile.history.map((item) => (
                <div key={item.id} className="bg-gray-50 p-3 rounded-xl">
                  ...
                  {item.boardHistory && item.boardHistory.length > 0 && (
                    <button onClick={() => {
                      if (item.analysisData) startHistoryAnalysis(item);
                      else setShowAnalysisModal(item);
                    }} className={`mt-2 px-3 py-2 w-full rounded-lg text-xs font-bold shadow-sm border active:scale-95 flex items-center justify-center ${item.analysisData ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-indigo-50 text-indigo-600 border-indigo-200'}`}>
                      <Bot size={14} className="mr-1"/> {item.analysisData ? '解析結果を見る' : 'この対局をAI解析する'}
                    </button>
                  )}
                </div>
              ))}
      ...
      <div className="w-full max-w-md h-full flex flex-col bg-gray-50 shadow-2xl relative">
        ...
        {analysisProgress && (
          <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
            <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-sm text-center">
              ...
              <h3 className="font-bold text-gray-800 text-lg mb-2">AI解析中...</h3>
              <p className="text-sm text-gray-600 mb-4">{analysisProgress.mode === 'deep' ? '超解析モードで深く読んでいます' : '通常解析を行っています'}</p>
              <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden">
                <div className="bg-blue-500 h-full transition-all duration-300" style={{ width: `${(analysisProgress.current / analysisProgress.total) * 100}%` }}></div>
              </div>
              <p className="text-xs font-bold text-gray-400 mt-2">{analysisProgress.current} / {analysisProgress.total} 手完了</p>
            </div>
          </div>
        )}
        ...
        {showAnalysisModal && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-in fade-in">
            <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-sm text-center">
              <Bot size={40} className="text-blue-500 mx-auto mb-3" />
              <h3 className="font-bold text-gray-800 text-lg mb-2">AI解析モード</h3>
              <p className="text-sm text-gray-600 mb-6">全手番を自動で解析し、評価値グラフと最善手を記録します。</p>
              <div className="space-y-3">
                <button onClick={() => runFullAnalysis(showAnalysisModal, 'normal')} className="w-full py-3 bg-blue-50 text-blue-600 font-bold rounded-xl active:scale-95">通常解析 (約10〜30秒)</button>
                <button onClick={() => runFullAnalysis(showAnalysisModal, 'deep')} className="w-full py-3 bg-indigo-50 text-indigo-600 font-bold rounded-xl active:scale-95">超解析 (約1〜3分/最大15手先)</button>
                <button onClick={() => setShowAnalysisModal(null)} className="w-full py-3 bg-gray-100 text-gray-600 font-bold rounded-xl mt-2">キャンセル</button>
              </div>
            </div>
          </div>
        )}
        ...
        {incomingInvitation && currentTab !== 'play' && (
          <div className="absolute inset-x-4 top-24 bg-white p-4 rounded-2xl shadow-2xl border border-gray-100 z-50 animate-in slide-in-from-top-4">
            <div className="flex items-start">
              <div className="p-3 bg-emerald-100 text-emerald-600 rounded-full mr-3"><Swords size={20}/></div>
              <div className="flex-1">
                <h4 className="font-bold text-gray-800 text-sm">{incomingInvitation.fromName} からの招待</h4>
                <p className="text-xs text-gray-500 mt-1">{incomingInvitation.mode === 'ranked' ? 'ランク戦' : 'ルーム戦'}に招待されました！</p>
              </div>
            </div>
            <div className="flex space-x-2 mt-4">
              <button onClick={declineInvitation} className="flex-1 py-2 bg-gray-100 text-gray-600 font-bold rounded-xl text-sm">拒否</button>
              <button onClick={acceptInvitation} className="flex-1 py-2 bg-emerald-500 text-white font-bold rounded-xl text-sm shadow-sm">参加する</button>
            </div>
          </div>
        )}
        ...
        {selectedFriend && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-in fade-in">
            <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-sm space-y-4 relative">
              ...
              <div className="text-center">
                <div className="w-16 h-16 bg-gray-100 rounded-full mx-auto mb-2 flex items-center justify-center border-2 border-gray-200"><User size={24} className="text-gray-400" /></div>
                <h3 className="font-black text-xl text-gray-800">{selectedFriend.name}</h3>
                <p className="text-sm font-bold text-gray-500 mt-1">Rate: {selectedFriend.rating}</p>
              </div>
              {selectedFriend.profile ? (
                <div className="grid grid-cols-2 gap-2 text-center text-sm font-bold mt-4">
                  ...
                  <div className="bg-gray-50 p-2 rounded-xl">勝率<br/><span className="text-lg text-emerald-600">{
                    (selectedFriend.profile.wins + selectedFriend.profile.losses + selectedFriend.profile.draws) > 0 
                    ? Math.round((selectedFriend.profile.wins / (selectedFriend.profile.wins + selectedFriend.profile.losses + selectedFriend.profile.draws)) * 100) 
                    : 0
                  }%</span></div>
              ...
              <div className="space-y-2 mt-4 pt-4 border-t border-gray-100">
                <p className="text-xs text-center text-gray-400 font-bold mb-2">このフレンドを対局に招待</p>
                { !selectedFriend.uid.startsWith('ai_bot_') && (
                  <button onClick={() => handleInvite(selectedFriend.uid, 'ranked')} className="w-full py-3 bg-emerald-50 text-emerald-600 font-bold rounded-xl flex items-center justify-center active:scale-95"><Swords size={18} className="mr-2"/> ランク戦に招待</button>
                )}
                <button onClick={() => handleInvite(selectedFriend.uid, 'room')} className="w-full py-3 bg-blue-50 text-blue-600 font-bold rounded-xl flex items-center justify-center active:scale-95"><Hash size={18} className="mr-2"/> ルーム戦に招待</button>
              </div>
            </div>
          </div>
        )}
