...
import { getFirestore, collection, doc, getDocs, addDoc, updateDoc, setDoc, getDoc, onSnapshot, runTransaction, arrayUnion } from "firebase/firestore";

// --- Firebase Configuration ---
// 環境変数(__firebase_config)がない場合は、ユーザー提供のconfigを使用します。
...
const firebaseConfig = typeof __firebase_config !== 'undefined' && __firebase_config ? JSON.parse(__firebase_config) : userProvidedConfig;
const app = initializeApp(firebaseConfig);
const auth = getAuth(app);
const db = getFirestore(app);

const appId = typeof __app_id !== 'undefined' ? __app_id : 'othello-app-production';
...
const getProfileRef = (uid) => doc(db, 'artifacts', appId, 'users', uid, 'profile');
...
const calcRatingChange = (myRate, oppRate, myStones, oppStones, isWin, isDraw) => {
  ...
  const bonus = isDraw ? 0 : stoneDiff * 0.15; 
...
const getBestMove = (board, color, depth) => {
  ...
  let bestMove = null;
  moves.sort(() => Math.random() - 0.5); // ランダム性で同じスコアなら手を散らす
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        // AI 初期化
        try {
          const aiCheck = await getDoc(getProfileRef('ai_bot_0'));
          if (!aiCheck.exists()) {
            for (let i = 0; i < 10; i++) {
              const uid = `ai_bot_${i}`;
              const aiName = `AI-Player-${i+1}`;
              const p = { name: aiName, rating: 1000, highestRating: 1000, highestRank: null, wins: 0, losses: 0, draws: 0, currentStreak: 0, history: [], theme: 'dark', isAI: true };
              await setDoc(getProfileRef(uid), p);
              await setDoc(getLeaderboardRef(uid), { uid, name: aiName, rating: p.rating, dailyDate: todayStr, dailyRating: 1000, dailyRank: 0, isAI: true, status: 'online' });
            }
          }
        } catch(e) {}
  ...
  const updateProfile = async (updates) => {
    ...
    setProfile(newProfile);
    if (user) {
      await updateDoc(getProfileRef(user.uid), updates);
      if (updates.rating !== undefined || updates.name !== undefined) {
        await setDoc(getLeaderboardRef(user.uid), { uid: user.uid, name: newProfile.name, rating: newProfile.rating }, { merge: true });
      }
    }
  };
  ...
  useEffect(() => {
      ...
      if (myIndex !== -1) {
        ...
        setProfile(prev => {
          if (!prev.highestRank || myRank < prev.highestRank) {
            updateDoc(getProfileRef(user.uid), { highestRank: myRank });
        ...
        if (myLbData.dailyDate !== todayStr) {
          setDoc(getLeaderboardRef(user.uid), { dailyDate: todayStr, dailyRating: myLbData.rating, dailyRank: myRank }, { merge: true });
    ...
    const interval = setInterval(async () => {
      if (Math.random() < 0.15) { // 15% probability per minute
        try {
          ...
          if (aiList.length >= 2) {
            ...
            await updateDoc(getLeaderboardRef(a1.uid), { status: 'playing' });
            await updateDoc(getLeaderboardRef(a2.uid), { status: 'playing' });
            ...
            setTimeout(async () => {
              ...
              for (let target of [{uid:a1.uid, change:a1Change, isWin:isA1Win&&!isDraw, isDraw}, {uid:a2.uid, change:a2Change, isWin:!isA1Win&&!isDraw, isDraw}]) {
                 const pSnap = await getDoc(getProfileRef(target.uid));
                 if (pSnap.exists()) {
                   const p = pSnap.data();
                   const newRating = Math.max(0, p.rating + target.change);
                   await updateDoc(getProfileRef(target.uid), {
                     rating: newRating, highestRating: Math.max(p.highestRating || 1000, newRating),
                     wins: p.wins + (target.isWin ? 1 : 0), losses: p.losses + (!target.isWin && !target.isDraw ? 1 : 0), draws: p.draws + (target.isDraw ? 1 : 0)
                   });
                   await updateDoc(getLeaderboardRef(target.uid), { rating: newRating, status: 'online' });
                 }
              }
            }, 15000); 
          }
    ...
    const processResult = async () => {
      ...
      if (oppUid?.startsWith('ai_bot_')) {
         ...
         if (aiSnap.exists()) {
           ...
           const aiChange = calcRatingChange(aiP.rating || 1000, profile.rating, oppStones, myStones, !isWin && !isDraw, isDraw);
           const newAiRating = Math.max(0, (aiP.rating || 1000) + aiChange);
           ...
           await updateDoc(getLeaderboardRef(oppUid), { rating: newAiRating });
         }
      }
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    const aiIndex = Math.floor(Math.random() * 10);
    const aiId = `ai_bot_${aiIndex}`;
    const aiProfileSnap = await getDoc(getProfileRef(aiId));
    if (aiProfileSnap.exists()) {
      const aiData = aiProfileSnap.data();
    ...
    try {
      ...
      if (foundGame) {
        ...
        const newGameRef = await addDoc(getGamesCollection(), {
          gameType: 'ranked', status: 'waiting', board: JSON.stringify(INITIAL_BOARD), turn: 'black',
  ...
  const joinRoom = async (roomPin) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!roomDoc.exists()) {
          transaction.set(roomRef, { gameType: 'room', status: 'waiting', board: JSON.stringify(INITIAL_BOARD), turn: 'black', player1: user.uid, player1Data: { name: profile.name }, player2: null, player2Data: null });
          role = 'player';
        } else {
  ...
  const startLocalMatch = () => {
    setGameState({ status: 'playing', board: INITIAL_BOARD, turn: 'black', player1Data: { name: 'Player 1' }, player2Data: { name: 'Player 2' } });
    setLocalHistory([INITIAL_BOARD]); setPlayMode('local'); setCurrentTab('play');
  };
  ...
  const handleLeaveGame = async () => {
    ...
    if (playMode === 'ranked' && gameState?.status === 'playing') {
      try { await updateDoc(getGameRef(gameId), { status: 'finished', resignedBy: user.uid }); } catch (e) { }
    }
      ...
      <div className="bg-gray-100 h-screen w-full flex justify-center font-sans overflow-hidden text-gray-800">
        <div className="w-full max-w-md h-full flex flex-col bg-gray-50 shadow-2xl relative items-center justify-center p-6 space-y-8 animate-in fade-in duration-500">
          ...
          <div className="w-full bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
             <div className="space-y-2"><label className="text-sm font-bold text-gray-700 ml-1">プレイヤー名を入力</label><input type="text" maxLength={10} value={initName} onChange={(e) => setInitName(e.target.value)} className="w-full p-4 bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-lg font-bold text-center" placeholder="名前 (10文字以内)" autoFocus /></div>
  ...
  // --- UI Screens ---
  const renderHome = () => (
    <div className="p-4 space-y-5 flex flex-col h-full animate-in fade-in duration-300">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-end justify-center z-50 animate-in fade-in">
          <div className="bg-white w-full h-[85%] rounded-t-3xl shadow-xl flex flex-col animate-in slide-in-from-bottom-full duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {leaderboardData.slice(0, 10).map((lbUser, idx) => {
                const isToday = lbUser.dailyDate === todayStr;
                const lbRank = idx + 1;
                const rankDiff = isToday && lbUser.dailyRank ? lbUser.dailyRank - lbRank : 0;
                const ratingDiff = isToday && lbUser.dailyRating ? lbUser.rating - lbUser.dailyRating : 0;

                return (
                  <div key={idx} className={`flex items-center justify-between p-4 rounded-xl border ${lbUser.uid === user?.uid ? 'bg-blue-50 border-blue-200 shadow-sm ring-1 ring-blue-200' : 'bg-white border-gray-100 shadow-sm'}`}>
  ...
  const renderFriends = () => {
    ...
    return (
      <div className="p-4 space-y-4 h-full animate-in fade-in duration-300">
        ...
        <div className="space-y-3">
          {friendList.length === 0 ? (
            ...
            friendList.map((fUser, idx) => (
              <div key={idx} className="flex items-center justify-between p-4 rounded-xl bg-white border border-gray-100 shadow-sm">
      ...
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm mb-6 relative">
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'black' ? activeTheme.light : ''}`}>
            {activeStamp?.sender === player1 && (<div className="absolute -top-10 bg-white px-3 py-1.5 rounded-2xl shadow-lg border border-gray-100 text-xs font-bold z-20 whitespace-nowrap animate-in zoom-in slide-in-from-bottom-2 duration-200">{activeStamp.text}<div className="absolute -bottom-1.5 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-white border-b border-r border-gray-100 rotate-45"></div></div>)}
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'white' ? activeTheme.light : ''}`}>
            {activeStamp?.sender === player2 && (<div className="absolute -top-10 bg-white px-3 py-1.5 rounded-2xl shadow-lg border border-gray-100 text-xs font-bold z-20 whitespace-nowrap animate-in zoom-in slide-in-from-bottom-2 duration-200">{activeStamp.text}<div className="absolute -bottom-1.5 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-white border-b border-r border-gray-100 rotate-45"></div></div>)}
  ...
  const renderProfile = () => {
    ...
    return (
      <div className="p-4 space-y-5 h-full animate-in fade-in duration-300 pb-8">
  ...
  const renderSettings = () => (
    <div className="p-4 space-y-6 h-full animate-in fade-in duration-300">
