...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    // ▼ 変更箇所: 10秒ごとに85%の確率で最大2組（4人）のAI同士の試合を発生させて加速
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    try {
      await updateDoc(getGameRef(gid), { 
        status: 'playing', 
        player2: aiId, 
        player2Data: { name: aiData.name || `AI-Player-${aiIndex+1}`, rating: aiData.rating || 1000, wins: aiData.wins||0, losses: aiData.losses||0, draws: aiData.draws||0 } 
      });
    } catch (e) { 
      console.error(e); 
      triggerCommError();
