...
// --- AI (Minimax) Logic ---
const getAiDepth = (rating, totalGames, isStrongAI = false) => {
...
function OthelloApp() {
  ...
  const [researchData, setResearchData] = useState({ topMoves: [], evaluation: 0, isAnalyzing: false });
        ...
        try {
          ...
          if (pSnap.exists()) {
            ...
            if (pData.resetVersion !== RESET_VERSION) {
              ...
              // ▼ 修正箇所: プレイヤーリセット時にも公開用戦績データをLeaderboardに反映
  ...
  const handleInitialRegistration = async () => {
    ...
    try {
      ...
      // ▼ 修正箇所: 初期登録時にも公開用戦績データをLeaderboardに反映
  ...
  const updateProfile = async (updates) => {
    ...
    if (user) {
      ...
      // ▼ 修正箇所: プロフィール更新時に公開データをLeaderboardにもマージして同期
  ...
  const handleUserClick = async (targetUser) => {
    ...
    try {
      ...
      // ▼ 修正箇所: 他人のデータはセキュリティに弾かれないよう公開領域(Leaderboard)から取得する
  ...
  // --- Game Sync ---
  useEffect(() => {
    if (!user || !gameId || playMode === 'local' || playMode === 'research') return;
  ...
  useEffect(() => {
    if (playMode === 'research' && gameState && gameState.status === 'playing') {
      setResearchData(prev => ({ ...prev, isAnalyzing: true }));
      const timerId = setTimeout(() => {
        const depth = isDeepAnalysis ? 7 : 5; 
        const moves = getSimulatedMoves(gameState.board, gameState.turn);
        if (moves.length === 0) {
          setResearchData({ topMoves: [], evaluation: 0, isAnalyzing: false });
          return;
        }
        ...
        setResearchData({ topMoves: evaluatedMoves.slice(0, 3), evaluation, isAnalyzing: false });
      }, 50); 
  ...
  useEffect(() => {
    ...
    if (aiId?.startsWith('ai_bot_') && (gameState.player1 === user?.uid || gameState.player2 === user?.uid)) {
      ...
      const depth = getAiDepth(referenceRating, totalGames, isStrongAI);
  ...
  const startResearchMode = () => {
    ...
    setCurrentTab('play');
    setResearchData({ topMoves: [], evaluation: 0, isAnalyzing: true });
  ...
  const handleMove = async (row, col, forceColor = null) => {
    ...
    if (playMode === 'local' || playMode === 'research') {
      ...
      setGameState({ ...gameState, board: newBoard, turn: nextTurn, status: nextStatus, passEvent }); 
      await updateDoc(getGameRef(gameId), { board: JSON.stringify(newBoard), turn: nextTurn, status: nextStatus, passEvent }).catch(e => { triggerCommError(); });
    }
  };

  const handleStamp = async (text) => {
    if (!gameId || playMode === 'local' || playMode === 'research' || playMode === 'room_spectator') return;
  ...
  const renderGame = () => {
    ...
    let myColor = (playMode === 'local' || playMode === 'research') ? turn : (player1 === user?.uid ? 'black' : 'white');
    ...
    return (
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
          ...
          <div className={`${activeTheme.board} p-1.5 rounded-lg shadow-2xl w-full max-w-[350px] aspect-square`}>
            <div className={`grid grid-cols-8 grid-rows-8 gap-0.5 h-full w-full ${activeTheme.grid} border-2 ${activeTheme.border}`}>
              {board.map((row, rIdx) => row.map((cell, cIdx) => {
                ...
                let rankIndex = -1;
                if (playMode === 'research' && !researchData.isAnalyzing) {
        ...
        {status === 'finished' && playMode !== 'research' && (
