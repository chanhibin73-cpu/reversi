...
function OthelloApp() {
  ...
  useEffect(() => {
    ...
    // ▼ AIシミュレーション: 20秒ごとに60%の確率で発生（対局時間は5秒に短縮）
        ...
        if (Math.random() < 0.60) { 
          ...
          if (aiList.length >= 2) {
            ...
            setTimeout(async () => {
              ...
              for (let target of [{uid:a1.uid, change:a1Change}, {uid:a2.uid, change:a2Change}]) {
                 // ▼ エラー修正：セキュリティルールで弾かれても確実にランキングのレートだけは書き込む
    ...
    const processResult = async () => {
      try {
        ...
        if (oppUid?.startsWith('ai_bot_')) {
           // ▼ エラー修正：ユーザーがAIに勝利/敗北した際も確実にAIのランキングレートを更新する
           const aiChange = calcRatingChange(oppData?.rating || 1000, profile.rating, oppStones, myStones, !isWin && !isDraw, isDraw);
           const newAiRating = Math.max(0, (oppData?.rating || 1000) + aiChange);
