...
function OthelloApp() {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (roomDoc.exists() && roomDoc.data().status === 'waiting') {
          ...
          // ▼ 修正: 空いている側（player1 または player2）に入る
          if (!roomData.player1) {
  ...
  const startAiMatchImmediately = async (gid) => {
    ...
    try {
      ...
      // ▼ 修正: プレイヤーが先手か後手かに応じて、AIを適切なスロットにセット
  ...
  const startRankedMatch = async () => {
    ...
    try {
      ...
      if (joined) {
        ...
        const initB = INITIAL_BOARD.map(r => [...r]);
        // ▼ 変更箇所: ランク戦の作成時に先手（P1）か後手（P2）かを50%でランダム決定
