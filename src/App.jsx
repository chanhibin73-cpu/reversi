...
const getMoveFromBoards = (prevBoard, currentBoard, turn) => {
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      if (prevBoard[r][c] === null && currentBoard[r][c] === turn) return { r, c };
    }
  }
  return null;
};
...
export default function OthelloApp() {
  ...
  useEffect(() => {
    const isAnalysisMode = playMode === 'research' || (playMode === 'room' && gameState?.allowAiAnalysis);
  ...
  const renderGame = () => {
    ...
    const isAnalysisOn = playMode === 'research' || playMode === 'history_analysis' || (playMode === 'room' && gameState?.allowAiAnalysis);
      ...
      <div className="p-4 flex flex-col h-full animate-in fade-in duration-300 relative">
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm mb-6 relative shrink-0">
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'black' ? activeTheme.light : ''} w-[100px]`}>
            ...
            {(playMode === 'ranked' || playMode === 'room_spectator') && player1Data && (
              <span className="text-[10px] text-gray-500 font-bold w-full text-center whitespace-nowrap">
                R:{player1Data?.rating || 1000}{player1Data?.currentStreak >= 2 ? ` 🔥${player1Data.currentStreak}` : ''}
              </span>
            )}
            <span className="text-xs font-black mt-1">{blackCount}</span>
          </div>
          ...
          <div className={`flex flex-col items-center p-2 rounded-lg relative ${turn === 'white' ? activeTheme.light : ''} w-[100px]`}>
            ...
            {(playMode === 'ranked' || playMode === 'room_spectator') && player2Data && (
              <span className="text-[10px] text-gray-500 font-bold w-full text-center whitespace-nowrap">
                R:{player2Data?.rating || 1000}{player2Data?.currentStreak >= 2 ? ` 🔥${player2Data.currentStreak}` : ''}
              </span>
            )}
            <span className="text-xs font-black mt-1">{whiteCount}</span>
          </div>
        </div>
        ...
        {playMode !== 'history_analysis' && (
          <div className="mt-6 flex space-x-3 relative shrink-0">
            ...
            {playMode !== 'room_spectator' && (<button onClick={() => { if(playMode === 'local' || playMode === 'research') setGameState({...gameState, status:'finished', resignedBy:gameState.turn}); else updateDoc(getGameRef(gameId), { status:'finished', resignedBy:user.uid }); }} disabled={status !== 'playing'} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl font-bold active:scale-95 disabled:opacity-50">投了</button>)}
            {(playMode === 'local' || playMode === 'research') && (<button onClick={() => { const h = localHistory.slice(0, -1); setLocalHistory(h); setGameState({ ...gameState, board: h[h.length - 1], turn: gameState.turn === 'black' ? 'white' : 'black', status: 'playing', passEvent: null }); }} disabled={localHistory.length <= 1 || (status !== 'playing' && playMode !== 'research')} className={`flex-1 text-white py-3 rounded-xl font-bold active:scale-95 disabled:opacity-50 ${activeTheme.primary}`}>待った</button>)}
          </div>
        )}
        ...
        {status === 'finished' && !isAnalysisMode && (
